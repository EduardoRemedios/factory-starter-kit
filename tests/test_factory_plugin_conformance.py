...
CODEX_RUNTIME = load_runtime(
    "factory_codex_runtime", REPO_ROOT / "plugins/factory/scripts/factory_plugin.py"
)
CLAUDE_RUNTIME = load_runtime(
    "factory_claude_runtime",
    REPO_ROOT / "plugins/factory-claude/scripts/factory_plugin.py",
)
...
class FactoryPluginConformanceTests(unittest.TestCase):
    ...
    def test_setup_semantics_differ_only_by_documented_claude_bridge(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            claude = CLAUDE_RUNTIME.evaluate_setup_plan(
                ...
                mode="brownfield",
                harness="claude",
                payload_root=REPO_ROOT / "plugins/factory-claude/payload",
                platform_name="darwin",
                python_version=(3, 11, 0),
            )
    ...
    def test_unverified_environment_has_stable_reason_on_both_packages(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(
                all(
                    item["reason_code"] == "FACTORY_ENVIRONMENT_UNVERIFIED"
                    for item in outputs
                )
            )
