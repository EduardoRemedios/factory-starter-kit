...
LAUNCHER = REPO_ROOT / "scripts/conductor-python"
...
class FactoryPythonLauncherTests(unittest.TestCase):
    ...
    def test_conductor_control_surfaces_use_launcher(self):
        surfaces = (
            "AGENTS.md",
            ...
            "plugin-src/conductor/skills/validate.md",
        )
    ...
    def test_conductorctl_self_routes_through_launcher_without_bytecode(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            scripts = root / "scripts"
            scripts.mkdir()
            for source in (REPO_ROOT / "scripts").iterdir():
                if source.is_file() and (source.suffix == ".py" or source.name in {"conductorctl", "conductor-python"}):
                    shutil.copy2(source, scripts / source.name)
            environment = os.environ.copy()
            environment.pop("PYTHONDONTWRITEBYTECODE", None)
            completed = subprocess.run(
                [str(scripts / "conductorctl"), "--help"],
                cwd=root,
                check=False,
                capture_output=True,
                text=True,
