...
SCRIPT = REPO_ROOT / "scripts/verify_conductor_cli_rollout.py"
...
class FactoryCliRolloutTests(unittest.TestCase):
    ...
    def test_mismatched_package_version_blocks_rollout(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            (root / "plugin-src/conductor/runtime").mkdir(parents=True)
            (root / ".claude-plugin/marketplace.json").write_text(
                json.dumps({"plugins": [{"name": "conductor"}]}),
                encoding="utf-8",
            )
            (root / "plugin-src/conductor/manifest.json").write_text(
                json.dumps({"version": "0.2.0"}),
                encoding="utf-8",
            )
            (root / "plugin-src/conductor/runtime/conductor_plugin.py").write_text(
