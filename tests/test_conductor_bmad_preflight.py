...
from tests.test_conductor_bmad_support import REPO_ROOT, runtime, seed_bmad, seed_factory


ADAPTER = REPO_ROOT / "plugin-src/conductor-bmad/project-adapter"
...
class FactoryBmadPreflightTests(unittest.TestCase):
    def setUp(self):
        ...
        run = self.root / "docs/Conductor/runs" / RUN_ID; run.mkdir(parents=True)
    ...
    def run_preflight(self):
        completed = subprocess.run(["python3", str(ADAPTER / "conductor_project_preflight"), "--run", RUN_ID, "--json"], cwd=self.root, text=True, capture_output=True)
    ...
    def test_valid_snapshot_passes(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_PREFLIGHT_PASS", payload["reason_code"])
    ...
    def test_prefixed_workflow_alias_promotes_to_preflight_pass(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_PREFLIGHT_PASS", self.run_preflight()["reason_code"])
    ...
    def test_seeded_project_preflight_does_not_write_bytecode(self):
        ...
        completed = subprocess.run(
            [sys.executable, "scripts/conductor_project_preflight", "--run", RUN_ID, "--json"],
    ...
    def test_review_qualifier_must_match_the_snapshot_manifest(self):
        ...
        self.assertEqual(
            "CONDUCTOR_BMAD_REVIEW_QUALIFIER_MISMATCH",
            self.run_preflight()["reason_code"],
        )

    def test_reserved_snapshot_id_fails_before_lookup(self):
        self.write_brief(snapshot="ReCeIpTs")
        self.assertEqual("CONDUCTOR_BMAD_SNAPSHOT_ID_RESERVED", self.run_preflight()["reason_code"])
    ...
    def test_snapshot_inventory_is_exact(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_SNAPSHOT_INVENTORY_INVALID", self.run_preflight()["reason_code"])

    def test_snapshot_symlink_and_mode_drift_fail(self):
        snapshot = self.root / "docs/upstream/bmad/example-v1"
        artifact = snapshot / "artifact.md"
        artifact.chmod(0o600)
        self.assertEqual("CONDUCTOR_BMAD_ARTIFACT_MODE_MISMATCH", self.run_preflight()["reason_code"])
    ...
    def test_direct_draft_citation_fails(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_DRAFT_CITATION", self.run_preflight()["reason_code"])
    ...
    def test_authority_and_freeze_fail_closed(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_AUTHORITY_INVALID", self.run_preflight()["reason_code"])
        self.write_brief(freeze="OPEN")
        self.assertEqual("CONDUCTOR_BMAD_CONTEXT_NOT_FROZEN", self.run_preflight()["reason_code"])

    def test_citation_and_artifact_hash_tamper_fail(self):
        self.write_brief(digest="0" * 64)
        self.assertEqual("CONDUCTOR_BMAD_CITATION_HASH_MISMATCH", self.run_preflight()["reason_code"])
        self.write_brief()
        (self.root / "docs/upstream/bmad/example-v1/artifact.md").write_text("tampered\n", encoding="utf-8")
        self.assertEqual("CONDUCTOR_BMAD_ARTIFACT_HASH_MISMATCH", self.run_preflight()["reason_code"])

    def test_intake_is_seed_only_and_uses_existing_preflight_seam(self):
        declaration = json.loads((self.root / "docs/Conductor/PROJECT_PREFLIGHT.json").read_text(encoding="utf-8"))
        ...
        self.assertTrue((self.root / "scripts/conductor_bmad_policy.py").is_file())
        self.assertTrue((self.root / "scripts/conductor_bmad_policy_lint").is_file())
        current = runtime.intake(self.root, "claude", None)
        self.assertEqual("CONDUCTOR_BMAD_INTAKE_CURRENT", current["reason_code"])
    ...
    def test_capability_and_policy_drift_fail_closed(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_POLICY_VERSION_DRIFT", self.run_preflight()["reason_code"])
    ...
    def test_uncovered_capability_fails_with_ci_reason(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_CAPABILITY_UNRECOGNIZED", self.run_preflight()["reason_code"])

    def test_existing_different_adapter_file_conflicts(self):
        target = self.root / "docs/Conductor/PROJECT_PREFLIGHT.json"
        target.write_text("{}\n", encoding="utf-8")
        payload = runtime.intake(self.root, "claude", None)
        self.assertEqual("CONDUCTOR_BMAD_INTAKE_CONFLICT", payload["reason_code"])
    ...
    def test_solution_context_claim_receipt_accepts_all_five_outcomes(self):
        ...
        receipt_path = self.root / f"docs/Conductor/runs/{RUN_ID}/BMAD_CLAIM_DISPOSITIONS.json"
    ...
    def test_solution_context_identity_authority_and_policy_fail_closed(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_EVIDENCE_TYPE_INVALID", self.run_preflight()["reason_code"])
        self.write_solution_brief(manifest, authority="BINDING")
        self.assertEqual("CONDUCTOR_BMAD_AUTHORITY_INVALID", self.run_preflight()["reason_code"])
        self.write_solution_brief(manifest, policy_version="1.0.0")
        self.assertEqual("CONDUCTOR_BMAD_SNAPSHOT_POLICY_VERSION_MISMATCH", self.run_preflight()["reason_code"])
        self.write_solution_brief(manifest, plan_id="0" * 64)
        self.assertEqual("CONDUCTOR_BMAD_PROMOTION_IDENTITY_MISMATCH", self.run_preflight()["reason_code"])
    ...
    def test_solution_context_duplicate_field_and_artifact_tamper_fail(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_BRIEF_FIELD_DUPLICATE", self.run_preflight()["reason_code"])
        self.write_solution_brief(manifest)
    ...
    def test_solution_context_claim_receipt_rejects_invalid_outcome_and_conflict_link(self):
        ...
        for outcome, link, expected in (
            ("IMPLEMENT", "NONE", "CONDUCTOR_BMAD_CLAIM_OUTCOME_INVALID"),
            ("CONFLICT", "NONE", "CONDUCTOR_BMAD_CLAIM_CONFLICT_LINK_MISSING"),
        ):
        ...
        self.assertEqual("CONDUCTOR_BMAD_CLAIM_RECEIPT_DUPLICATE", self.run_preflight()["reason_code"])

        base["claims"] = base["claims"][:1]
        base["snapshot"] = {**base["snapshot"], "snapshot_id": "different-v1"}
        receipt_path.write_text(json.dumps({**base, "aggregate_sha256": runtime.digest_bytes(runtime.canonical(base))}), encoding="utf-8")
        self.assertEqual("CONDUCTOR_BMAD_CLAIM_RECEIPT_SNAPSHOT_MISMATCH", self.run_preflight()["reason_code"])
