...
from tests import test_factory_plugin_composition as composition
from tests.test_factory_plugin_status import inventory


LIVE_REQUESTED = os.environ.get("FACTORY_PLUGIN_COMPOSITION_LIVE") == "1"
...
def run_claude(
    ...
    binary = environment.get("FACTORY_CLAUDE_BIN")
    if not binary:
        raise AssertionError("FACTORY_CLAUDE_BIN is required")
...
class FactoryPluginCompositionLiveTests(unittest.TestCase):
    def test_two_lane_dependency_and_namespaced_routing(self):
        ...
        required = (
            "FACTORY_CLAUDE_BIN",
            "FACTORY_CLAUDE_VERSION_PREFIX",
            "FACTORY_BMAD_LIVE_ROOT",
            "FACTORY_BMAD_LIVE_CONFIG_DIR",
            "FACTORY_BMAD_LIVE_CANDIDATE_ROOT",
            "FACTORY_BMAD_LIVE_EVIDENCE_ROOT",
            "FACTORY_BMAD_PREFLIGHT_EVIDENCE",
            "FACTORY_BMAD_PREFLIGHT_SHA256",
            "FACTORY_BMAD_PERMISSION_RULE",
            "FACTORY_RELEASE_VERSION",
        )
        missing = [name for name in required if not environment.get(name)]
        self.assertEqual([], missing, f"missing live inputs: {missing}")
        verified = subprocess.run(
            [
                str(composition.REPO_ROOT / "scripts/factory-python"),
                "scripts/verify_factory_bmad_live_preflight.py",
                "--verify-verdict",
                environment["FACTORY_BMAD_PREFLIGHT_EVIDENCE"],
                "--expected-verdict-sha256",
                environment["FACTORY_BMAD_PREFLIGHT_SHA256"],
                "--expected-claude-bin",
                environment["FACTORY_CLAUDE_BIN"],
                "--expected-config-root",
                environment["FACTORY_BMAD_LIVE_CONFIG_DIR"],
                "--expected-evidence-root",
                environment["FACTORY_BMAD_LIVE_EVIDENCE_ROOT"],
                "--expected-candidate-root",
                environment["FACTORY_BMAD_LIVE_CANDIDATE_ROOT"],
                "--expected-live-root",
                environment["FACTORY_BMAD_LIVE_ROOT"],
                "--expected-permission-rule",
                environment["FACTORY_BMAD_PERMISSION_RULE"],
                "--expected-release-version",
                environment["FACTORY_RELEASE_VERSION"],
            ],
        ...
        self.assertIn(environment["FACTORY_CLAUDE_VERSION_PREFIX"], version.stdout)

        base = Path(environment["FACTORY_BMAD_LIVE_ROOT"])
        config = Path(environment["FACTORY_BMAD_LIVE_CONFIG_DIR"])
        candidate = Path(environment["FACTORY_BMAD_LIVE_CANDIDATE_ROOT"])
        ...
        original_factory_package = composition.FACTORY_PACKAGE
        composition.FACTORY_PACKAGE = candidate / "plugins/factory-claude"
        try:
            marketplace, _, companion = composition.make_marketplace(base)
        finally:
            composition.FACTORY_PACKAGE = original_factory_package
        ...
        session_only = [
            "--plugin-dir", str(candidate / "plugins/factory-claude"),
            "--plugin-dir", str(companion),
            "--print", "--output-format", "json", "--no-session-persistence",
            "--setting-sources", "", "--tools", "Bash",
            "--allowedTools", environment["FACTORY_BMAD_PERMISSION_RULE"],
            "--permission-mode", "dontAsk",
        ]
        doctor = run_claude([*session_only, "/factory:doctor Return only state, reason_code, and mutations."], cwd=project, environment=routing_environment)
        self.assertEqual(0, doctor.returncode, doctor.stdout + doctor.stderr)
        self.assertIn("FACTORY_PROJECT_NOT_CONFIGURED", json.loads(doctor.stdout)["result"])
        greenfield = run_claude([*session_only, "/factory:greenfield Return only state, reason_code, plan_id, and mutations; do not enumerate planned files."], cwd=project, environment=routing_environment)
