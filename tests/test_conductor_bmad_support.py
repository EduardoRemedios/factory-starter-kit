...
RUNTIME_PATH = REPO_ROOT / "plugin-src/conductor-bmad/runtime/conductor_bmad.py"
sys.dont_write_bytecode = True
SPEC = importlib.util.spec_from_file_location("conductor_bmad_runtime", RUNTIME_PATH)
...
def seed_factory(root: Path) -> None:
    (root / "docs/Conductor").mkdir(parents=True, exist_ok=True)
    (root / "docs/Conductor/ARCHITECTURE.md").write_text("# Factory\n", encoding="utf-8")
    (root / "scripts").mkdir(exist_ok=True)
    (root / "scripts/conductorctl").write_text("#!/bin/sh\n", encoding="utf-8")
