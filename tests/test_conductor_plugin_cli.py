...
def runtime_for(name: str, fixture_root: Path) -> Path:
    if name == "claude":
        return REPO_ROOT / "plugins/conductor-claude/scripts/conductor_plugin.py"
    if name == "codex":
        return REPO_ROOT / "plugins/conductor/scripts/conductor_plugin.py"
    package = fixture_root / "authored-package"
    runtime = package / "scripts/conductor_plugin.py"
    runtime.parent.mkdir(parents=True)
    shutil.copy2(REPO_ROOT / "plugin-src/conductor/runtime/conductor_plugin.py", runtime)
    shutil.copytree(REPO_ROOT / "plugins/conductor/payload", package / "payload")
    return runtime


def run_cli(runtime: Path, cwd: Path, *args: str) -> tuple[subprocess.CompletedProcess[str], dict]:
    output_args = ("--json", *args)
    completed = subprocess.run(
        [sys.executable, str(runtime), *output_args],
        cwd=cwd,
        check=False,
        capture_output=True,
        text=True,
    ...
    def test_codex_harness_cli_rejects_claude_local_settings(self):
        for runtime_name in RUNTIME_NAMES:
            with self.subTest(runtime=runtime_name), tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertEqual(
                    "CONDUCTOR_GREENFIELD_NOT_EMPTY", payload["reason_code"]
                )
                self.assertEqual(
    ...
    def test_greenfield_defaults_to_empty_current_directory(self):
        for runtime_name in RUNTIME_NAMES:
            with self.subTest(runtime=runtime_name), tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertEqual(0, completed.returncode, completed.stderr)
                self.assertEqual("PLAN_READY", payload["state"])
                self.assertEqual("CONDUCTOR_PLAN_READY", payload["reason_code"])
    ...
    def test_greenfield_rejects_nonempty_current_directory(self):
        for runtime_name in RUNTIME_NAMES:
            with self.subTest(runtime=runtime_name), tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertEqual("CONDUCTOR_GREENFIELD_NOT_EMPTY", payload["reason_code"])
                self.assertEqual(
                    "choose_an_empty_target_or_remove_non_project_harness_content",
                    payload["next_legal_action"],
                )
    ...
    def test_doctor_outside_git_still_requires_git_root(self):
        for runtime_name in RUNTIME_NAMES:
            with self.subTest(runtime=runtime_name), tempfile.TemporaryDirectory() as temp_dir:
                ...
                self.assertEqual(2, completed.returncode, completed.stderr)
                self.assertEqual("BLOCKED", payload["state"])
                self.assertEqual("CONDUCTOR_GIT_ROOT_REQUIRED", payload["reason_code"])
                self.assertEqual([], payload["mutations"])
                self.assertEqual(before, inventory(root))
