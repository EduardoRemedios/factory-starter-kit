...
from tests.test_factory_bmad_support import REPO_ROOT, seed_bmad, seed_factory, seed_git
...
@unittest.skipUnless(os.environ.get("FACTORY_BMAD_LIVE_CLAUDE") == "1", "isolated authenticated Claude proof not authorized")
class FactoryBmadClaudeLiveTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.claude = Path(os.environ["FACTORY_CLAUDE_BIN"])
        if not cls.claude.is_file():
            raise AssertionError("explicit Claude Code binary unavailable")
        cls.candidate = Path(os.environ["FACTORY_BMAD_LIVE_CANDIDATE_ROOT"])
        cls.base = Path(os.environ["FACTORY_BMAD_LIVE_ROOT"])
        cls.config = Path(os.environ["FACTORY_BMAD_LIVE_CONFIG_DIR"])
        if cls.base.exists() or cls.base.is_symlink():
            raise AssertionError("FACTORY_BMAD_LIVE_ROOT must be absent")
        verified = subprocess.run(
            [
                str(REPO_ROOT / "scripts/factory-python"),
                "scripts/verify_factory_bmad_live_preflight.py",
                "--verify-verdict",
                os.environ["FACTORY_BMAD_PREFLIGHT_EVIDENCE"],
                "--expected-verdict-sha256",
                os.environ["FACTORY_BMAD_PREFLIGHT_SHA256"],
                "--expected-claude-bin",
                os.environ["FACTORY_CLAUDE_BIN"],
                "--expected-config-root",
                os.environ["FACTORY_BMAD_LIVE_CONFIG_DIR"],
                "--expected-evidence-root",
                os.environ["FACTORY_BMAD_LIVE_EVIDENCE_ROOT"],
                "--expected-candidate-root",
                os.environ["FACTORY_BMAD_LIVE_CANDIDATE_ROOT"],
                "--expected-live-root",
                os.environ["FACTORY_BMAD_LIVE_ROOT"],
                "--expected-permission-rule",
                os.environ["FACTORY_BMAD_PERMISSION_RULE"],
                "--expected-release-version",
                os.environ["FACTORY_RELEASE_VERSION"],
            ],
            cwd=REPO_ROOT,
            text=True,
            capture_output=True,
        ...
        if cls.config.is_symlink():
            raise AssertionError("FACTORY_BMAD_LIVE_CONFIG_DIR must not be a symlink")
    ...
    def invoke(self, root: Path, prompt: str, *, tools: str = "") -> str:
        command = [
            str(self.claude),
            "--plugin-dir", str(self.candidate / "plugins/factory-claude"),
            "--plugin-dir", str(self.candidate / "plugins/factory-bmad-claude"),
    ...
    def test_direct_allowed_prohibited_unknown_and_inactive_paths(self):
        ...
        self.assertIn("FACTORY_BMAD_WORKFLOW_PROHIBITED", prohibited)
        unknown = self.invoke(active, "/bmad-future-autonomous")
        self.assertIn("FACTORY_BMAD_WORKFLOW_PROHIBITED", unknown)
        allowed = self.invoke(active, "/bmad-product-brief")
        self.assertNotIn("FACTORY_BMAD_WORKFLOW_PROHIBITED", allowed)
        inactive = self.invoke(self.repository(factory=False), "/bmad-architecture")
        self.assertNotIn("FACTORY_BMAD_WORKFLOW_PROHIBITED", inactive)
    ...
    def test_model_initiated_skill_is_denied(self):
        ...
        self.assertIn("FACTORY_BMAD_WORKFLOW_PROHIBITED", output)
