...
from conductor_stage_lint import lint_stage  # noqa: E402
...
class FactoryProjectPreflightTests(unittest.TestCase):
    def test_absent_declaration_preserves_existing_behavior(self) -> None:
        ...
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_NOT_DECLARED", result["reason_code"])
    ...
    def test_valid_pass_records_separate_bounded_metadata(self) -> None:
        ...
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_PASS", result["reason_code"])
    ...
    def test_malformed_declaration_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            self._write(root / "docs/Conductor/PROJECT_PREFLIGHT.json", "not json\n")
            result = run_project_preflight(root, RUN_ID)
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_DECLARATION_INVALID", result["reason_code"])
    ...
    def test_missing_and_non_executable_commands_fail_closed(self) -> None:
        ...
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_COMMAND_MISSING", missing["reason_code"])
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_COMMAND_INVALID", invalid["reason_code"])
    ...
    def test_timeout_and_nonzero_exit_fail_closed(self) -> None:
        ...
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_TIMEOUT", timeout["reason_code"])
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_EXIT_NONZERO", nonzero["reason_code"])
    ...
    def test_oversized_output_fails_without_retaining_excess(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_project_preflight(root, RUN_ID)
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_OUTPUT_TOO_LARGE", result["reason_code"])
    ...
    def test_malformed_ambiguous_and_explicit_fail_outputs_are_distinct(self) -> None:
        ...
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_OUTPUT_MALFORMED", malformed["reason_code"])
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_OUTPUT_AMBIGUOUS", ambiguous["reason_code"])
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_FAILED", failed["reason_code"])
    ...
    def test_absolute_evidence_path_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = run_project_preflight(root, RUN_ID)
        self.assertEqual("CONDUCTOR_PROJECT_PREFLIGHT_EVIDENCE_PATH_INVALID", result["reason_code"])
    ...
    def test_stage_a_requires_pass_evidence_only_when_declared(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self._write(
                ...
                "project_preflight: PASS\nreason_code: CONDUCTOR_PROJECT_PREFLIGHT_PASS\n",
            )
    ...
    def test_root_operator_contract_has_exact_order(self) -> None:
        ...
        planner = (REPO_ROOT / ".agents/skills/conductor-root-planner/SKILL.md").read_text(
            encoding="utf-8"
        )
    ...
    def _declaration(self, root: Path, *, timeout_seconds: int = 60) -> None:
        self._write(
            root / "docs/Conductor/PROJECT_PREFLIGHT.json",
    ...
    def _command(self, root: Path, body: str, *, executable: bool = True) -> None:
        path = root / "scripts/conductor_project_preflight"
        self._write(
            path,
