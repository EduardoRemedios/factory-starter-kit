...
from tests.test_conductor_bmad_support import runtime
...
class FactoryBmadPromotionTests(unittest.TestCase):
    ...
    def test_stale_source_blocks(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_PLAN_APPROVAL_MISMATCH", payload["reason_code"])
    ...
    def test_optional_review_qualifier_is_plan_bound_and_immutable(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_PLAN_APPROVAL_MISMATCH", stale["reason_code"])
    ...
    def test_traversal_symlink_and_downstream_workflow_block(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_PATH_INVALID", traversal["reason_code"])
        ...
        self.assertEqual("CONDUCTOR_BMAD_SYMLINK_REJECTED", symlink["reason_code"])
        legacy_solution = runtime.promote(self.root, self.args(workflow="architecture"))
        self.assertEqual("CONDUCTOR_BMAD_SOLUTION_CONTEXT_REQUIRED", legacy_solution["reason_code"])
        ...
        self.assertEqual("CONDUCTOR_BMAD_WORKFLOW_PROHIBITED", obsolete["reason_code"])
    ...
    def test_immutable_conflict_blocks(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_SNAPSHOT_IMMUTABLE_CONFLICT", payload["reason_code"])
    ...
    def test_reserved_snapshot_ids_are_casefolded(self):
        for snapshot_id in ("latest", "LATEST", "receipts", "RECEIPTS", "Install-Receipts", "INSTALL-RECEIPTS"):
            with self.subTest(snapshot_id=snapshot_id):
                ...
                self.assertEqual("CONDUCTOR_BMAD_SNAPSHOT_ID_RESERVED", payload["reason_code"])
    ...
    def test_snapshot_mode_drift_blocks_reuse_and_rollback(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_SNAPSHOT_IMMUTABLE_CONFLICT", runtime.promote(self.root, self.args())["reason_code"])
        self.assertEqual("CONDUCTOR_BMAD_ROLLBACK_DIGEST_MISMATCH", runtime.rollback(self.root, applied["receipt"], None)["reason_code"])
    ...
    def test_changed_snapshot_refuses_rollback(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_ROLLBACK_DIGEST_MISMATCH", payload["reason_code"])
    ...
    def test_solution_context_is_immutable_when_draft_changes(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_SNAPSHOT_IMMUTABLE_CONFLICT", runtime.promote(self.root, self.solution_args())["reason_code"])
    ...
    def test_solution_context_requires_typed_evidence_only_plan_identity(self):
        self.assertEqual("CONDUCTOR_BMAD_AUTHORITY_INVALID", runtime.promote(self.root, self.solution_args(authority="BINDING"))["reason_code"])
        self.assertEqual("CONDUCTOR_BMAD_EVIDENCE_TYPE_INVALID", runtime.promote(self.root, self.solution_args(evidence_type="SPEC"))["reason_code"])
        self.assertEqual("CONDUCTOR_BMAD_PLAN_IDENTITY_INVALID", runtime.promote(self.root, self.solution_args(plan_identity=""))["reason_code"])
        ...
        self.assertEqual("CONDUCTOR_BMAD_WORKFLOW_PROHIBITED", runtime.promote(self.root, self.solution_args(workflow="create-architecture"))["reason_code"])

    def test_solution_context_rejects_empty_or_symlinked_content(self):
        empty = self.root / "_bmad-output/empty"
        empty.mkdir()
        self.assertEqual("CONDUCTOR_BMAD_SOURCE_EMPTY", runtime.promote(self.root, self.solution_args(source="_bmad-output/empty"))["reason_code"])
        outside = self.root / "outside.md"
        outside.write_text("outside\n", encoding="utf-8")
        (self.root / "_bmad-output/solution/link.md").symlink_to(outside)
        self.assertEqual("CONDUCTOR_BMAD_SYMLINK_REJECTED", runtime.promote(self.root, self.solution_args())["reason_code"])
    ...
    def test_solution_context_supersession_is_exact_and_preserves_prior_snapshot(self):
        ...
        self.assertEqual("CONDUCTOR_BMAD_SUPERSESSION_INVALID", invalid["reason_code"])
