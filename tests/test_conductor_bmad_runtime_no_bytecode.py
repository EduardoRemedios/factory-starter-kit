...
RUNTIME_ROOT = REPO_ROOT / "plugin-src/conductor-bmad/runtime"
...
class FactoryBmadRuntimeNoBytecodeTests(unittest.TestCase):
    def run_candidate(self, label: str) -> tuple[subprocess.CompletedProcess[str], list[Path]]:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for name in ("conductor_bmad.py", "conductor_bmad_policy.py"):
                shutil.copy2(RUNTIME_ROOT / name, scripts / name)
            environment = os.environ.copy()
            environment.pop("PYTHONDONTWRITEBYTECODE", None)
            environment.pop("PYTHONPYCACHEPREFIX", None)
            completed = subprocess.run(
                [sys.executable, str(scripts / "conductor_bmad.py"), "--help"],
                env=environment,
                text=True,
                capture_output=True,
                check=False,
    ...
    def test_runtime_candidate_is_self_protecting_before_policy_import(self):
        for label in ("authored", "conductor-bmad-package", "conductor-bmad-claude-package"):
