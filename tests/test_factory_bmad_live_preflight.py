...
SOURCE_COUPLING = REPO_ROOT / "tests/plugin_fixtures/factory_bmad_025_source_coupling.json"
...
class FactoryBmadLivePreflightTests(unittest.TestCase):
    def invoke(
        ...
        *,
        ...
        (candidate / "plugin-src/factory").mkdir(parents=True)
        ...
        (candidate / "plugin-src/factory/manifest.json").write_text(
            json.dumps({"version": "0.2.5"}), encoding="utf-8"
        )
        (candidate / "plugin-src/factory-bmad/manifest.json").write_text(
            json.dumps({"version": "0.2.5", "factory_dependency": "~0.2.5"}),
            encoding="utf-8",
        )
        (candidate / ".claude-plugin/marketplace.json").write_text(
            json.dumps(
                {
                    "plugins": [
                        {"name": "factory", "version": "0.2.5"},
                        {"name": "factory-bmad", "version": "0.2.5"},
                    ]
                }
            ),
            encoding="utf-8",
    ...
    def test_live_preflight_fixtures_are_release_owned(self):
        for path in (CONTRACT, SOURCE_COUPLING):
            with self.subTest(path=path):
                ...
                self.assertNotIn("docs/Factory/runs/", relative)
        ...
        with tempfile.TemporaryDirectory() as temporary:
            for rule in ("Bash(python3:*)", "Bash(*)"):
                with self.subTest(rule=rule):
                    ...
                    self.assertIn("FACTORY_BMAD_PERMISSION_RULE_INVALID", verdict["reason_codes"])
    ...
    def test_realpath_overlap_is_blocked(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(1, completed.returncode)
            verdict = json.loads(output.read_text(encoding="utf-8"))
            self.assertIn("FACTORY_BMAD_ROOT_OVERLAP", verdict["reason_codes"])
    ...
    def test_missing_journey_root_is_blocked_before_live_use(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            verdict = json.loads(output.read_text(encoding="utf-8"))
            self.assertIn(
                "FACTORY_BMAD_JOURNEY_ROOT_INVALID", verdict["reason_codes"]
            )

    def test_live_assets_have_no_stale_permission_cleanup_or_config_drop(self):
        composition = (REPO_ROOT / "tests/test_factory_plugin_composition_live.py").read_text()
        bmad_live = (REPO_ROOT / "tests/test_factory_bmad_claude_live.py").read_text()
        shell = (REPO_ROOT / "scripts/verify_factory_bmad_claude_composition.sh").read_text()
        ...
        self.assertIn('FACTORY_PLUGIN_COMPOSITION_LIVE") == "1"', composition)
        self.assertNotIn("TemporaryDirectory", bmad_live)
        self.assertNotIn("trap 'rm -rf", shell)
        self.assertNotIn("mktemp -d", shell)
        self.assertIn("FACTORY_BMAD_PREFLIGHT_EVIDENCE", shell)
        self.assertIn("FACTORY_BMAD_LIVE_ROOT", shell)
        self.assertLess(
