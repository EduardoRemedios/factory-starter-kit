...
class FactoryBmadPolicyParityTests(unittest.TestCase):
    ...
    def test_unknown_returns_same_stable_reason(self):
        ...
        audit = runtime.capability_audit(root, "claude")
        lint = runtime.policy_lint(root, "claude")
        self.assertEqual("FACTORY_BMAD_CAPABILITY_UNRECOGNIZED", audit["reason_code"])
        self.assertEqual(audit["reason_code"], lint["reason_code"])

    def test_ci_callable_policy_lint_cli(self):
        root = self.root()
        completed = subprocess.run(
            [sys.executable, str(REPO_ROOT / "plugin-src/factory-bmad/runtime/factory_bmad.py"), "--root", str(root), "--json", "policy-lint", "--harness", "claude"],
    ...
    def test_seeded_project_lint_uses_same_reason(self):
        ...
        completed = subprocess.run(
            [sys.executable, "scripts/factory_bmad_policy_lint"],
    ...
    def test_generated_policy_copies_match_authored_source(self):
        authored = REPO_ROOT / "plugin-src/factory-bmad/runtime/factory_bmad_policy.py"
        self.assertEqual(authored.read_bytes(), (REPO_ROOT / "plugins/factory-bmad/scripts/factory_bmad_policy.py").read_bytes())
        self.assertEqual(authored.read_bytes(), (REPO_ROOT / "plugins/factory-bmad-claude/scripts/factory_bmad_policy.py").read_bytes())
        self.assertEqual(authored.read_bytes(), (REPO_ROOT / "plugins/factory-bmad/assets/project-adapter/factory_bmad_policy.py").read_bytes())
        self.assertEqual(authored.read_bytes(), (REPO_ROOT / "plugins/factory-bmad-claude/assets/project-adapter/factory_bmad_policy.py").read_bytes())
