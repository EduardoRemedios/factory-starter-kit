...
CLOSEOUT = load_module(
    "conductor_execution_closeout",
    REPO_ROOT / "scripts/conductor_execution_closeout.py",
)
RUNTIME = load_module(
    "conductor_plugin_runtime_closeout",
    REPO_ROOT / "plugin-src/conductor/runtime/conductor_plugin.py",
)
...
class ExecutionCloseoutTests(unittest.TestCase):
    def make_root(self) -> Path:
        ...
        shutil.copyfile(REPO_ROOT / "scripts/conductor_execution_closeout.py", validator)
        return root
    ...
    def make_run(self, run_id: str = "RUN_20260805_0000_fixture") -> Path:
        run_root = self.root / "docs/Conductor/runs" / run_id
    ...
    def test_ec01_valid_review_ready_and_idempotent_record(self):
        ...
        self.assertEqual([f"docs/Conductor/runs/{run.name}/EXECUTION_CLOSEOUT.json"], first["mutations"])
    ...
    def test_ec02_valid_no_go(self):
        ...
        self.assertEqual("CONDUCTOR_EXECUTION_NO_GO", output["reason_code"])
    ...
    def test_ec05_malformed_unknown_and_duplicate_ids_fail(self):
        ...
        output = RUNTIME.evaluate_progress(self.root, run_id=run.name)
        self.assertEqual("CONDUCTOR_EXECUTION_CLOSEOUT_INVALID", output["reason_code"])
        malformed.unlink()
        write(malformed, '{"schema":"one","schema":"two"}')
        output = RUNTIME.evaluate_progress(self.root, run_id=run.name)
        self.assertEqual(
            "CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", output["blocker"]
        )
        malformed.unlink()
        unknown = self.draft(run)
        unknown["unknown"] = True
        self.assert_invalid(run, unknown, "CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID")
        duplicate = self.draft(run)
        duplicate["verification_results"][1]["id"] = "VM-001"
        self.assert_invalid(run, duplicate, "CONDUCTOR_EXECUTION_CLOSEOUT_DUPLICATE_ID")

    def test_ec06_wrong_run_or_sprint_fails(self):
        run = self.make_run()
        wrong_run = self.draft(run)
        wrong_run["run_id"] = "RUN_20260805_9999_wrong"
        self.assert_invalid(run, wrong_run, "CONDUCTOR_EXECUTION_CLOSEOUT_IDENTITY_MISMATCH")
        wrong_sprint = self.draft(run)
        wrong_sprint["sprint_id"] = "SPRINT_20260805_999"
        self.assert_invalid(run, wrong_sprint, "CONDUCTOR_EXECUTION_CLOSEOUT_IDENTITY_MISMATCH")
    ...
    def test_ec07_wrong_pack_or_authorization_digest_fails(self):
        ...
        for field in ("pack_manifest", "execution_authorization"):
            ...
            self.assert_invalid(run, draft, "CONDUCTOR_EXECUTION_CLOSEOUT_PIN_MISMATCH")
    ...
    def test_ec08_missing_or_unsafe_evidence_fails(self):
        ...
        self.assert_invalid(run, missing, "CONDUCTOR_EXECUTION_CLOSEOUT_EVIDENCE_MISSING")
        unsafe = self.draft(run)
        unsafe["verification_results"][0]["evidence_path"] = "../outside.txt"
        self.assert_invalid(run, unsafe, "CONDUCTOR_EXECUTION_CLOSEOUT_UNSAFE_PATH")
        ...
        self.assert_invalid(run, symlinked, "CONDUCTOR_EXECUTION_CLOSEOUT_UNSAFE_PATH")

    def test_symlinked_run_root_ancestors_block_validation_and_recording(self):
        for ancestor in ("docs", "docs/Conductor", "docs/Conductor/runs"):
            with self.subTest(ancestor=ancestor), tempfile.TemporaryDirectory() as temp_dir:
                ...
                try:
                    ...
                    self.assertEqual(
                        "CONDUCTOR_EXECUTION_CLOSEOUT_UNSAFE_PATH",
    ...
    def test_ec09_evidence_drift_blocks_progress(self):
        ...
        self.assertEqual("CONDUCTOR_EXECUTION_CLOSEOUT_EVIDENCE_DIGEST_MISMATCH", output["blocker"])
    ...
    def test_ec10_incomplete_or_duplicate_micro_sprints_fail(self):
        ...
        self.assert_invalid(run, incomplete, "CONDUCTOR_EXECUTION_CLOSEOUT_MICRO_SPRINT_MISMATCH")
        duplicate = self.draft(run)
    ...
    def test_ec11_outcome_evidence_contradiction_fails(self):
        ...
        self.assert_invalid(run, draft, "CONDUCTOR_EXECUTION_CLOSEOUT_OUTCOME_CONTRADICTION")

    def test_ec12_closeout_cannot_grant_authority(self):
        run = self.make_run()
        draft = self.draft(run)
        draft["authority_grants"] = ["release"]
        self.assert_invalid(run, draft, "CONDUCTOR_EXECUTION_CLOSEOUT_AUTHORITY_VIOLATION")
    ...
    def test_ec13_recorded_closeout_survives_restoration_and_archival(self):
        ...
        self.assertEqual("CONDUCTOR_EXECUTION_REVIEW_READY", progress["reason_code"])
    ...
    def test_ec14_record_still_requires_live_execution_enabled_mode(self):
        ...
        self.assertEqual(
            "CONDUCTOR_EXECUTION_CLOSEOUT_IDENTITY_MISMATCH",
    ...
    def test_ec15_restored_closeout_fails_without_matching_archived_authorization(self):
        ...
        self.assertEqual(
            "CONDUCTOR_EXECUTION_CLOSEOUT_PIN_MISMATCH",
            caught.exception.reason_code,
        )
