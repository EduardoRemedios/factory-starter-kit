...
from tests.test_factory_bmad_support import REPO_ROOT, runtime, seed_bmad, seed_git


FACTORY_RUNTIME_PATH = REPO_ROOT / "plugin-src/factory/runtime/factory_plugin.py"
FACTORY_SPEC = importlib.util.spec_from_file_location("factory_bmad_pilot_factory_runtime", FACTORY_RUNTIME_PATH)
factory_runtime = importlib.util.module_from_spec(FACTORY_SPEC)
assert FACTORY_SPEC.loader is not None
FACTORY_SPEC.loader.exec_module(factory_runtime)
FACTORY_PAYLOAD = REPO_ROOT / "plugins/factory/payload"
...
class FactoryBmadSingleRepositoryPilotTests(unittest.TestCase):
    ...
    def adopt_factory(self, root: Path, mode: str):
        plan = factory_runtime.evaluate_setup_plan(
            root,
            mode=mode,
            harness="claude",
            payload_root=FACTORY_PAYLOAD,
            platform_name="darwin",
            python_version=(3, 12, 0),
        )
        self.assertEqual("PLAN_READY", plan["state"], plan)
        applied = factory_runtime.apply_setup_plan(
            root,
            plan=plan,
            approved_plan_id=plan["plan_id"],
            payload_root=FACTORY_PAYLOAD,
        )
        self.assertEqual("FACTORY_SETUP_APPLIED", applied["reason_code"], applied)

    def install_bmad_with_factory(self, root: Path):
        if os.environ.get("FACTORY_BMAD_LIVE_INSTALLER") == "1":
    ...
    def test_greenfield_neither_journey(self):
        ...
        self.assertEqual("FACTORY_ONLY", runtime.doctor(root, "claude")["state"])
