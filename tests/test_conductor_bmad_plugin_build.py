...
SOURCE = REPO_ROOT / "plugin-src/conductor-bmad"
PACKAGES = {"codex": REPO_ROOT / "plugins/conductor-bmad", "claude": REPO_ROOT / "plugins/conductor-bmad-claude"}


class FactoryBmadPluginBuildTests(unittest.TestCase):
    def test_packages_are_current(self):
        completed = subprocess.run([sys.executable, "scripts/build_conductor_bmad_plugins.py", "--check"], cwd=REPO_ROOT, text=True, capture_output=True)
    ...
    def test_manifest_and_skill_contract(self):
        ...
        self.assertEqual([{"name": "conductor", "version": "~0.2.5"}], claude["dependencies"])
        self.assertEqual("0.2.5", claude["version"])
    ...
    def test_claude_package_has_both_hook_paths_without_manifest_hook_field(self):
        ...
        for event in ("UserPromptExpansion", "PreToolUse"):
            ...
            self.assertIn("${CLAUDE_PLUGIN_ROOT}/scripts/conductor_bmad.py", command)
    ...
    def test_no_factory_core_or_bmad_runtime_is_vendored(self):
        for package in PACKAGES.values():
            ...
            self.assertFalse((package / "docs/Conductor").exists())
    ...
    def test_generic_factory_payload_excludes_companion_scripts(self):
        prohibited = {
            ...
            "verify_conductor_bmad_claude_composition.sh",
            "verify_conductor_bmad_live_pilot.sh",
            "verify_conductor_bmad_release.sh",
        }
        for package in (REPO_ROOT / "plugins/conductor", REPO_ROOT / "plugins/conductor-claude"):
    ...
    def test_marketplaces_expose_separate_companion(self):
        ...
        self.assertEqual({"conductor", "conductor-bmad"}, {item["name"] for item in agents["plugins"]})
        self.assertEqual({"conductor", "conductor-bmad"}, {item["name"] for item in claude["plugins"]})
