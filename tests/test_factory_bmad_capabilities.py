...
from tests.test_factory_bmad_support import runtime, seed_bmad, seed_factory, seed_git, seed_nested_bmad
...
class FactoryBmadCapabilityTests(unittest.TestCase):
    ...
    def test_nested_bmad_audit_reports_loop_before_activation(self):
        ...
        self.assertEqual("FACTORY_BMAD_NESTED_LAYOUT", payload["layout_reason_code"])
    ...
    def test_unsafe_layouts_fail_closed_with_stable_reasons(self):
        ...
        cases.append((nested, "nested_active", "FACTORY_BMAD_NESTED_LAYOUT"))

        combined = self.root(); seed_bmad(combined, capabilities=True); seed_bmad(combined / "bmad", capabilities=True)
        cases.append((combined, "canonical_and_nested", "FACTORY_BMAD_MULTIPLE_ACTIVE_ROOTS"))
        ...
        cases.append((ambiguous, "ambiguous", "FACTORY_BMAD_MANIFEST_AMBIGUOUS"))

        partial = self.root(); (partial / "_bmad").mkdir()
        cases.append((partial, "partial", "FACTORY_BMAD_PARTIAL_STATE"))
        ...
        cases.append((linked, "active_root_symlink", "FACTORY_BMAD_ACTIVE_ROOT_SYMLINK"))
        ...
        for root, expected_state, expected_reason in cases:
            with self.subTest(expected_state):
                ...
                self.assertEqual("FACTORY_BMAD_NON_CANONICAL_LAYOUT", doctor["reason_code"])
                self.assertEqual(expected_reason, doctor["evidence"]["layout_reason_code"])
                audit = runtime.capability_audit(root, "claude")
                self.assertEqual("FACTORY_BMAD_NON_CANONICAL_LAYOUT", audit["reason_code"])
                ...
                self.assertEqual("FACTORY_BMAD_ENFORCEMENT_ACTIVE_UNSAFE_LAYOUT", activation["reason_code"])
    ...
    def test_fixed_legacy_archive_is_inert_beside_canonical_root(self):
        ...
        self.assertEqual("FACTORY_ONLY", runtime.doctor(archive_only, "claude")["state"])
