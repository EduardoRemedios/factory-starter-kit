...
from scripts import build_conductor_plugins
...
class FactoryPluginBuildTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        build_conductor_plugins.write_packages(generated_root, cls.manifest)
        global PACKAGES
        PACKAGES = {
    ...
    def test_release_version_is_aligned_everywhere(self):
        ...
        self.assertEqual("0.3.4", version)
        ...
        runtime = (SOURCE_ROOT / "runtime/conductor_plugin.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_packages_have_exact_public_skill_set(self):
        ...
        codex_ids = {
            path.name.removeprefix("conductor-")
    ...
    def test_shared_skill_bodies_are_equal(self):
        for skill in self.manifest["skills"]:
            ...
            codex = body_without_frontmatter(
                PACKAGES["codex"] / "skills" / f"conductor-{skill_id}" / "SKILL.md"
            )
    ...
    def test_second_generation_is_clean(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            build_conductor_plugins.write_packages(regenerated, self.manifest)
            after = {
    ...
    def test_project_installation_state_is_not_packaged(self):
        for package in PACKAGES.values():
            self.assertFalse(
                (package / "payload/docs/Conductor/installation").exists()
            )

    def test_shared_runtime_is_equal(self):
        source = (SOURCE_ROOT / "runtime" / "conductor_plugin.py").read_bytes()
        for package in PACKAGES.values():
            self.assertEqual(source, (package / "scripts" / "conductor_plugin.py").read_bytes())
    ...
    def test_safe_python_launcher_bytes_and_mode_are_preserved(self):
        source = REPO_ROOT / "scripts/conductor-python"
        for package in PACKAGES.values():
            generated = package / "payload/scripts/conductor-python"
    ...
    def test_project_owned_lifecycle_payloads_use_dedicated_neutral_seeds(self):
        ...
        for destination in (
            ...
            Path("docs/Conductor/SCRATCHPAD.md"),
        ):
            self.assertNotEqual(destination, expected[destination])

        payload_sources = {
            destination: source
            for destination, source, _classification in build_conductor_plugins.payload_sources()
        }
        self.assertEqual(
            expected[Path("docs/Conductor/SCRATCHPAD.md")],
            payload_sources[Path("docs/Conductor/SCRATCHPAD.md")],
        )
        self.assertNotEqual(
            (REPO_ROOT / "docs/Conductor/SCRATCHPAD.md").read_bytes(),
            (REPO_ROOT / expected[Path("docs/Conductor/SCRATCHPAD.md")]).read_bytes(),
        )
