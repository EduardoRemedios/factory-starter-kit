...
SPEC = importlib.util.spec_from_file_location("conductor_plugin_adapter_runtime", RUNTIME_PATH)
...
class FactoryPluginAdapterTests(unittest.TestCase):
    ...
    def test_existing_factory_role_skills_coexist(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            write_skill(root, "conductor-root-planner", "conductor-root-planner")
            write_skill(root, "conductor-purple-gate", "conductor-purple-gate")
            before = inventory(root)
            output = RUNTIME.evaluate_skill_coexistence(
                root,
                plugin_skill_names={"conductor-doctor", "conductor-progress"},
            )
            self.assertEqual("COMPATIBLE", output["state"])
            self.assertEqual("CONDUCTOR_SKILLS_COMPATIBLE", output["reason_code"])
    ...
    def test_exact_skill_name_collision_halts_without_deletion(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            write_skill(root, "local-planner", "conductor-root-planner")
            before = inventory(root)
            output = RUNTIME.evaluate_skill_coexistence(
                root,
                plugin_skill_names={"conductor-root-planner"},
            )
            self.assertEqual("BLOCKED", output["state"])
            self.assertEqual("CONDUCTOR_SKILL_COLLISION", output["reason_code"])
            self.assertEqual([], output["automatic_deletions"])
            self.assertEqual(before, inventory(root))
