...
CONDUCTOR_PACKAGE = REPO_ROOT / "plugins/conductor-claude"
CONDUCTOR_SKILLS = {
    "conductor:doctor",
...
RUNTIME_SPEC = importlib.util.spec_from_file_location(
    "conductor_plugin_composition_runtime",
    CONDUCTOR_PACKAGE / "scripts/conductor_plugin.py",
)
...
def claude_with_plugin_cli() -> str:
    candidates = [
        os.environ.get("CONDUCTOR_CLAUDE_BIN"),
        shutil.which("claude"),
...
def dependency_result(
    *, conductor_version: str | None, enabled: bool, required: str = "=0.3.3"
) -> dict[str, object]:
    if conductor_version is None or not enabled:
        return {"outcome": "halt", "reason": "dependency-unsatisfied"}
    if required != f"={conductor_version}":
        return {
            "outcome": "halt",
...
def make_marketplace(base: Path) -> tuple[Path, Path, Path]:
        ...
        json.dumps(
            {
                ...
                "dependencies": [{"name": "conductor", "version": "=0.3.3"}],
            },
            indent=2,
        )
        + "\n",
    ...
    write(
        ...
        "Invoke `/conductor:doctor` and preserve its result.\n",
    )
    write(
    ...
    for tag in ("factory--v0.3.3", "companion-fixture--v1.0.0"):
...
class FactoryPluginCompositionTests(unittest.TestCase):
    ...
    def test_cp01_exact_dependency_and_local_release_tag(self):
        ...
        self.assertIn("factory--v0.3.3", tags)
        self.assertEqual(
            {"outcome": "pass", "dependency_resolved": True, "enabled": True},
            dependency_result(conductor_version="0.3.3", enabled=True),
        )

    def test_cp02_cp03_cp04_dependency_failures_halt(self):
        self.assertEqual(
            "dependency-unsatisfied",
            dependency_result(conductor_version=None, enabled=True)["reason"],
        )
        self.assertEqual(
            "dependency-unsatisfied",
            dependency_result(conductor_version="0.3.3", enabled=False)["reason"],
        )
        self.assertEqual(
            "dependency-version-unsatisfied",
            dependency_result(conductor_version="0.1.0", enabled=True)["reason"],
        )

    def test_cp05_exact_namespaced_skill_inventory(self):
        self.assertEqual(CONDUCTOR_SKILLS, plugin_skills(self.factory))
        self.assertEqual(
            {"companion-fixture:doctor"}, plugin_skills(self.companion)
        )
        self.assertFalse(CONDUCTOR_SKILLS & plugin_skills(self.companion))
    ...
    def test_cp06_doctor_is_read_only_and_preserves_base_reason(self):
        ...
        self.assertEqual("CONDUCTOR_PROJECT_NOT_CONFIGURED", output["reason_code"])
