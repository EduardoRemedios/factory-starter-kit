...
from scripts.factory_host_capability import (
    artifact_path,
    discover_artifact,
    host_identity,
    validate_artifact,
...
from scripts.factory_pack_lint import (
...
class HostCapabilityValidatorTests(unittest.TestCase):
    ...
    def test_missing_usr_bin_test_fails_on_current_macos_host(self) -> None:
        ...
        self.assertEqual(result["reason_code"], "FACTORY_HOST_CAPABILITY_PATH_MISSING")
    ...
    def test_relative_duplicate_and_unknown_kind_fail_schema(self) -> None:
        ...
        self.assertEqual(result["reason_code"], "FACTORY_HOST_CAPABILITY_SCHEMA_INVALID")
    ...
    def test_host_mismatch_and_deferred_target_block(self) -> None:
        ...
        self.assertEqual(result["reason_code"], "FACTORY_HOST_CAPABILITY_HOST_MISMATCH")
        document["posture"] = "DEFERRED_TARGET"
        result = validate_document(document, run_id="RUN_TEST")
        self.assertEqual(result["reason_code"], "FACTORY_HOST_CAPABILITIES_DEFERRED_TARGET")
    ...
    def test_file_directory_and_executable_kinds(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(executable_result["reason_code"], "FACTORY_HOST_CAPABILITY_NOT_EXECUTABLE")
    ...
    def test_discovery_writes_only_exact_pack_artifact_after_validation(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            pack = root / "docs/Factory/runs/RUN_TEST/pack"
            ...
            self.assertEqual(result["status"], "PASS")
            self.assertEqual(result["mutations"], ["docs/Factory/runs/RUN_TEST/pack/host_capabilities.json"])
...
class HostCapabilityPackLintTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.run = self.root / "docs/Factory/runs/RUN_TEST"
    ...
    def test_absolute_command_without_declaration_fails(self) -> None:
        ...
        self.assertEqual(result["reason_code"], "FACTORY_HOST_CAPABILITY_DECLARATION_INVALID")
    ...
    def test_declared_absolute_command_requires_artifact(self) -> None:
        ...
        result = evaluate_host_capability_contract(self.root, self.run)
        self.assertEqual(result["reason_code"], "FACTORY_HOST_CAPABILITIES_MISSING")
    ...
    def test_unknown_id_and_path_mismatch_fail(self) -> None:
        ...
        self.write_artifact("/bin/test")
        result = evaluate_host_capability_contract(self.root, self.run)
        self.assertEqual(result["reason_code"], "FACTORY_HOST_CAPABILITY_DECLARATION_MISMATCH")

    def test_relative_command_without_artifact_is_compatible(self) -> None:
        self.write_manifest(command="./scripts/factory-python -V", capability_ids=None)
        result = evaluate_host_capability_contract(self.root, self.run)
        self.assertEqual(result["reason_code"], "FACTORY_HOST_CAPABILITIES_NOT_REQUIRED")
