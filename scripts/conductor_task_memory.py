...
def load_runbooks(root: Path) -> dict[str, dict[str, Any]]:
    ...
    if not runbook_dir.exists():
        raise TaskMemoryError("Runbook directory is missing. Run `scripts/conductorctl memory-init` first.")
    ...
    if not runbooks:
        raise TaskMemoryError("No runbooks found. Run `scripts/conductorctl memory-init` to seed defaults.")
...
def load_journal(root: Path) -> list[dict[str, Any]]:
    ...
    if not journal_file.exists():
        raise TaskMemoryError("Task journal is missing. Run `scripts/conductorctl memory-init` first.")
