...
ROOT="${CONDUCTOR_BMAD_VM011_ROOT:-}"
APPROVAL="${CONDUCTOR_BMAD_VM011_APPROVED_PLAN_ID:-}"
RUNTIME="plugins/conductor-bmad-claude/scripts/conductor_bmad.py"
if [[ -z "$ROOT" || ! -d "$ROOT" ]]; then
  echo "BLOCKED: set CONDUCTOR_BMAD_VM011_ROOT to the prepared disposable repository" >&2
  exit 2
fi
if [[ -z "$APPROVAL" ]]; then
  echo "BLOCKED: set CONDUCTOR_BMAD_VM011_APPROVED_PLAN_ID to the exact human-approved bootstrap plan" >&2
  exit 2
fi
...
test -s "$MANIFEST"
test -s "$ROOT/docs/Conductor/ARCHITECTURE.md"
test -x "$ROOT/scripts/conductorctl"
test -d "$ROOT/.git"
