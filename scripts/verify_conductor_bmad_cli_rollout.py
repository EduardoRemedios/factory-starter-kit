            ...
            checks.append(
                check(
                    ...
                    (
                        ...
                        "uninstall conductor-bmad and factory, run `claude plugin prune`, "
...
def marketplace_checks(root: Path) -> list[dict[str, str]]:
    ...
    factory = load_json(root / "plugins/conductor-claude/.claude-plugin/plugin.json")
    companion = load_json(root / "plugins/conductor-bmad-claude/.claude-plugin/plugin.json")
    hooks = load_json(root / "plugins/conductor-bmad-claude/hooks/hooks.json")
    source = load_json(root / "plugin-src/conductor-bmad/manifest.json")
    ...
    if marketplace is None:
        ...
        status = "PASS" if {"conductor", "conductor-bmad"} <= names else "BLOCKED"
    ...
    expected_dependency = [{"name": "conductor", "version": f"~{EXPECTED_RELEASE}"}]
    if versions == {EXPECTED_RELEASE} and dependency == expected_dependency:
        checks.append(check("package_versions", "PASS", f"factory/conductor-bmad {EXPECTED_RELEASE} with dependency ~{EXPECTED_RELEASE}"))
    else:
        checks.append(check("package_versions", "BLOCKED", f"versions={sorted(str(item) for item in versions)} dependency={dependency!r}"))

    bmad_version = source_constant(root / "plugin-src/conductor-bmad/runtime/conductor_bmad.py", "BMAD_VERSION")
    ...
    expected = 'python3 "${CLAUDE_PLUGIN_ROOT}/scripts/conductor_bmad.py" hook'
