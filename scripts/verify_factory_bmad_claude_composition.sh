...
CLAUDE_BIN="${FACTORY_CLAUDE_BIN:?FACTORY_CLAUDE_BIN is required}"
LIVE_ROOT="${FACTORY_BMAD_LIVE_ROOT:?FACTORY_BMAD_LIVE_ROOT is required}"
CONFIG="${FACTORY_BMAD_LIVE_CONFIG_DIR:?FACTORY_BMAD_LIVE_CONFIG_DIR is required}"
PREFLIGHT="${FACTORY_BMAD_PREFLIGHT_EVIDENCE:?FACTORY_BMAD_PREFLIGHT_EVIDENCE is required}"
PREFLIGHT_SHA="${FACTORY_BMAD_PREFLIGHT_SHA256:?FACTORY_BMAD_PREFLIGHT_SHA256 is required}"
RELEASE_VERSION="${FACTORY_RELEASE_VERSION:?FACTORY_RELEASE_VERSION is required}"
CANDIDATE_ROOT="${FACTORY_BMAD_LIVE_CANDIDATE_ROOT:?FACTORY_BMAD_LIVE_CANDIDATE_ROOT is required}"
EVIDENCE_ROOT="${FACTORY_BMAD_LIVE_EVIDENCE_ROOT:?FACTORY_BMAD_LIVE_EVIDENCE_ROOT is required}"
PERMISSION_RULE="${FACTORY_BMAD_PERMISSION_RULE:?FACTORY_BMAD_PERMISSION_RULE is required}"
PYTHON_LAUNCHER="${FACTORY_PYTHON_LAUNCHER:-$PWD/scripts/factory-python}"
...
if [[ -e "$LIVE_ROOT" || -L "$LIVE_ROOT" ]]; then
  echo "BLOCKED: FACTORY_BMAD_LIVE_ROOT must be absent" >&2
  exit 2
fi
...
"$PYTHON_LAUNCHER" scripts/verify_factory_bmad_live_preflight.py \
...
"$CLAUDE_BIN" --version
PLUGIN_VALIDATION="$($CLAUDE_BIN plugin validate --strict "$CANDIDATE_ROOT/plugins/factory-bmad-claude")"
...
cp -R "$CANDIDATE_ROOT/plugins/factory-claude" "$MARKET/plugins/factory-claude"
cp -R "$CANDIDATE_ROOT/plugins/factory-bmad-claude" "$MARKET/plugins/factory-bmad-claude"
...
git -C "$MARKET" tag "factory-bmad--v$RELEASE_VERSION"
git -C "$PROJECT" init -q

"$CLAUDE_BIN" plugin marketplace add "$MARKET"
"$CLAUDE_BIN" plugin install factory-bmad@factory-starter-kit
...
companion = next(item for name, item in names.items() if name.startswith("factory-bmad@"))
...
if DISABLE_OUTPUT="$($CLAUDE_BIN plugin disable factory@factory-starter-kit 2>&1)"; then
  echo "BLOCKED: Claude allowed a required Factory dependency to be disabled" >&2
  exit 2
fi
grep -q 'still required by factory-bmad' <<<"$DISABLE_OUTPUT"
