...
def load_preimages(values: list[str], expected_roles: set[str], reasons: list[str]) -> dict[str, dict[str, Any]]:
    ...
    for raw in values:
        ...
        if not separator or role in preimages or path.is_symlink() or not path.is_file():
            reasons.append("CONDUCTOR_BMAD_PROTECTED_PREIMAGE_INVALID")
            continue
        try:
...
def validate_candidate(candidate: Path, release: str, reasons: list[str]) -> None:
    manifests = (
        candidate / "plugin-src/conductor/manifest.json",
        candidate / "plugin-src/conductor-bmad/manifest.json",
    ...
    try:
        ...
        reasons.append("CONDUCTOR_BMAD_CANDIDATE_RELEASE_INVALID")
        return
    versions = {factory.get("version"), companion.get("version")}
    versions.update(item.get("version") for item in marketplace.get("plugins", []))
    if versions != {release} or companion.get("conductor_dependency") != f"~{release}":
        reasons.append("CONDUCTOR_BMAD_CANDIDATE_RELEASE_INVALID")
...
def validate_source_coupling(path: Path, reasons: list[str]) -> str:
    try:
        ...
        reasons.append("CONDUCTOR_BMAD_SOURCE_COUPLING_INVALID")
        return ""
    ...
    for relative in candidates:
        ...
        if candidate.parts[0] != "tests":
            ...
            if any(stale in text for stale in stale_values):
                reasons.append("CONDUCTOR_BMAD_STALE_LIVE_LITERAL")
    return hashlib.sha256(path.read_bytes()).hexdigest()


def write_verdict(output: Path, value: dict[str, Any]) -> None:
    if output.exists() or output.is_symlink():
        raise RuntimeError("CONDUCTOR_BMAD_PREFLIGHT_EVIDENCE_EXISTS")
...
def check(args: argparse.Namespace) -> int:
    ...
    for name, path in raw_roots.items():
        if has_symlink_component(path.absolute()):
            reasons.append(f"CONDUCTOR_BMAD_ROOT_SYMLINK:{name}")
    ...
    for index, left_name in enumerate(names):
        for right_name in names[index + 1 :]:
            if paths_overlap(roots[left_name], roots[right_name]):
                reasons.append("CONDUCTOR_BMAD_ROOT_OVERLAP")
    evidence_root = roots["evidence"]
    output = args.output.resolve(strict=False)
    try:
        output.relative_to(evidence_root)
    except ValueError:
        reasons.append("CONDUCTOR_BMAD_EVIDENCE_PATH_ESCAPE")
...
def verify(args: argparse.Namespace) -> int:
    ...
    if path.is_symlink() or not path.is_file():
        print("CONDUCTOR_BMAD_PREFLIGHT_EVIDENCE_INVALID", file=sys.stderr)
        return 2
    value = json.loads(path.read_text(encoding="utf-8"))
    ...
    if mismatch:
        print("CONDUCTOR_BMAD_PREFLIGHT_EVIDENCE_MISMATCH", file=sys.stderr)
        return 2
