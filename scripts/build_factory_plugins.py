...
def write_packages(staging_root: Path, manifest: dict[str, Any]) -> None:
    for platform in PACKAGE_ROOTS:
        ...
        for skill in manifest["skills"]:
            skill_id = skill["id"]
            generated_name = f"factory-{skill_id}" if platform == "codex" else skill_id
        ...
        ownership = {
            ...
            "generated_from": "plugin-src/factory",
        }
...
def build(check: bool) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="factory-plugin-build-") as temp_dir:
