...
FORBIDDEN_PREFIXES = ("artifacts/", "docs/Factory/runs/")
PUBLIC_PREFIXES = (
    "docs/adapters/bmad/", "plugin-src/factory-bmad/",
    "plugins/factory-bmad/", "plugins/factory-bmad-claude/",
)
PUBLIC_EXACT = {
    ".agents/plugins/marketplace.json", ".claude-plugin/marketplace.json",
    "docs/CHANGELOG.md", "docs/PROJECT_STATE.md", "docs/ROADMAP.md",
    "scripts/build_factory_plugins.py", "scripts/build_factory_bmad_plugins.py",
    "scripts/verify_factory_bmad_publication.py",
    "plugins/factory/payload/OWNERSHIP.json",
    "plugins/factory-claude/payload/OWNERSHIP.json",
    "plugins/factory/payload/docs/CHANGELOG.md",
    "plugins/factory/payload/docs/PROJECT_STATE.md",
    "plugins/factory/payload/docs/ROADMAP.md",
    "plugins/factory-claude/payload/docs/CHANGELOG.md",
    "plugins/factory-claude/payload/docs/PROJECT_STATE.md",
    "plugins/factory-claude/payload/docs/ROADMAP.md",
}
PRIVACY_RULES = {
...
PRIVACY_EXCEPTIONS = (
    ("scripts/verify_factory_bmad_publication.py", "local_home", "/Users/[^/", "Privacy-rule definition only."),
    ("scripts/verify_factory_bmad_publication.py", "local_home", "/Users/person/", "Exact negative-fixture definition only."),
    ("scripts/verify_factory_bmad_publication.py", "customer", "Symphony", "Privacy-rule definition only."),
    ("scripts/verify_factory_bmad_publication.py", "customer", "AuditEdge", "Privacy-rule definition only."),
    ("scripts/verify_factory_bmad_publication.py", "customer", "website_sales", "Privacy-rule definition only."),
    ("scripts/verify_factory_bmad_publication.py", "enterprise_profile", "Claude Enterprise", "Privacy-rule definition only."),
    ("tests/test_factory_bmad_docs_privacy.py", "customer", "Symphony", "Negative privacy-test fixture only."),
    ("tests/test_factory_bmad_docs_privacy.py", "customer", "AuditEdge", "Negative privacy-test fixture only."),
    ("tests/test_factory_bmad_docs_privacy.py", "customer", "website_sales", "Negative privacy-test fixture only."),
    ("tests/test_factory_bmad_docs_privacy.py", "enterprise_profile", "Claude Enterprise", "Negative privacy-test fixture only."),
    ("tests/test_factory_bmad_publication.py", "local_home", "/Users/person/", "Negative privacy-test fixture only."),
    ("tests/test_factory_bmad_publication.py", "customer", "AuditEdge", "Negative privacy-test fixture only."),
)
...
def public_path(path: str) -> bool:
    return (
        ...
        or (path.startswith("scripts/verify_factory_bmad_") and path.endswith(".sh"))
        or (path.startswith("tests/test_factory_bmad_") and path.endswith(".py"))
...
def build_manifest(root: Path, output: Path) -> dict[str, object]:
    ...
    core = {
        "schema": "factory.publication-manifest.v1",
...
def validate_manifest(root: Path, manifest: dict[str, object], contract: dict[str, object] | None = None) -> None:
    if manifest.get("schema") != "factory.publication-manifest.v1" or manifest.get("authority_grants") != []:
...
def source_identity_snapshot(root: Path) -> dict[str, object]:
    return {
        "schema": "factory.publication-baseline.v1",
...
def validate_source_identity(root: Path, baseline: dict[str, object]) -> dict[str, object]:
    if baseline.get("schema") != "factory.publication-baseline.v1":
...
def materialize(root: Path, manifest: dict[str, object], baseline: dict[str, object], output: Path) -> dict[str, object]:
    ...
    payload = {"schema": "factory.publication-candidate.v1", "worktree": str(candidate), "commit": commit, "base_commit": manifest["base_commit"], "manifest_sha256": manifest["aggregate_sha256"], "public_paths": sorted(item["path"] for item in manifest["entries"]), "source_identity": identity, "authority_grants": []}
    write(output, payload)
    return payload
...
def validate_candidate(candidate: dict[str, object]) -> tuple[Path, str]:
    ...
    if set(candidate) != required or candidate.get("schema") != "factory.publication-candidate.v1" or candidate.get("authority_grants") != []:
...
def closeout_check(root: Path, baseline: dict[str, object], candidate: dict[str, object]) -> dict[str, object]:
    ...
    protected = ("plugin-src/factory-bmad/", "plugins/factory-bmad/", "plugins/factory-bmad-claude/", "artifacts/verification/factory_bmad_release_gate_repair/", "docs/Factory/runs/RUN_20260811_0839_factory_bmad_release_gate_repair/")
