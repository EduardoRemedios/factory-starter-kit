...
SOURCE_ROOT = REPO_ROOT / "plugin-src/factory-bmad"
PACKAGE_ROOTS = {
    "codex": REPO_ROOT / "plugins/factory-bmad",
    "claude": REPO_ROOT / "plugins/factory-bmad-claude",
}
...
def write_package(root: Path, platform: str, manifest: dict[str, Any]) -> None:
    ...
    for skill in manifest["skills"]:
        skill_id = skill["id"]
        generated = f"factory-bmad-{skill_id}" if platform == "codex" else skill_id
    ...
    shutil.copyfile(SOURCE_ROOT / "runtime/factory_bmad.py", root / "scripts/factory_bmad.py")
    shutil.copyfile(SOURCE_ROOT / "runtime/factory_bmad_policy.py", root / "scripts/factory_bmad_policy.py")
    for source in sorted((SOURCE_ROOT / "project-adapter").iterdir()):
        if source.is_file():
            shutil.copy2(source, root / "assets/project-adapter" / source.name)
    shutil.copyfile(
        SOURCE_ROOT / "runtime/factory_bmad_policy.py",
        root / "assets/project-adapter/factory_bmad_policy.py",
    )
    if platform == "codex":
        plugin = {
        ...
        plugin = {
            "name": manifest["name"], "version": manifest["version"],
            "description": manifest["description"], "author": manifest["author"],
            "dependencies": [{"name": "factory", "version": manifest["factory_dependency"]}],
        }
    ...
    ownership = {
        ...
        "generated_from": "plugin-src/factory-bmad", "files": files,
    }
...
def build(check: bool) -> bool:
    ...
    with tempfile.TemporaryDirectory(prefix="factory-bmad-build-") as temporary:
