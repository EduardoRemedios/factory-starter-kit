...
def resolve_git_root(start: Path) -> Path:
    ...
    if completed.returncode != 0:
        raise ValueError("CONDUCTOR_GIT_ROOT_REQUIRED")
...
def environment_problem(
    *, harness: str, platform_name: str, python_version: tuple[int, ...]
    ...
    if harness not in SUPPORTED_HARNESSES:
        return "CONDUCTOR_HARNESS_UNSUPPORTED"
    if platform_name != SUPPORTED_PLATFORM or python_version < (3, 11):
        return "CONDUCTOR_ENVIRONMENT_UNVERIFIED"
    return None
...
def project_compatibility(root: Path) -> tuple[str, list[str]]:
    required = (
        Path("AGENTS.md"),
        Path("docs/Conductor/ARCHITECTURE.md"),
        Path("docs/Conductor/ORCHESTRATION.md"),
...
def evaluate_skill_coexistence(
    ...
    if collisions:
        return result(
            state="BLOCKED",
            reason_code="CONDUCTOR_SKILL_COLLISION",
            next_legal_action="rename_or_remove_the_collision_only_with_owner_approval",
            **details,
        )
    return result(
        state="COMPATIBLE",
        reason_code="CONDUCTOR_SKILLS_COMPATIBLE",
...
def evaluate_doctor(
    ...
    *,
    ...
    installed_factory_version = (
        installation_state["conductor_version"] if installation_state else None
    )
    ...
    coexistence = (
        ...
        if harness == "codex"
            ...
            "state": "NAMESPACED",
            "reason_code": "CONDUCTOR_SKILLS_NAMESPACED",
            "collisions": [],
            "automatic_deletions": [],
            "migration_guidance": (
    ...
    if compatibility == "NOT_CONFIGURED":
        return result(
            ...
            reason_code="CONDUCTOR_PROJECT_NOT_CONFIGURED",
    ...
    if compatibility == "INCOMPLETE":
        return result(
            state="BLOCKED",
            reason_code="CONDUCTOR_PROJECT_INCOMPLETE",
            next_legal_action="review_missing_factory_prerequisites",
            **details,
        )
    return result(
        state="READY",
        reason_code="CONDUCTOR_DOCTOR_OK",
...
def select_run(root: Path, run_id: str | None) -> Path | None:
    runs_root = root / "docs" / "Conductor" / "runs"
    if run_id:
        if "/" in run_id or "\\" in run_id or run_id in {".", ".."}:
            raise ValueError("CONDUCTOR_UNSAFE_RUN_ID")
...
def evaluate_execution_closeout(root: Path, run_root: Path) -> dict[str, Any] | None:
    ...
    if not closeout.exists():
        return None
    validator = root / "scripts" / "conductor_execution_closeout.py"
    if not validator.is_file():
        return {
            "status": "FAIL",
            "reason_code": "CONDUCTOR_EXECUTION_CLOSEOUT_VALIDATOR_MISSING",
            "detail": "scripts/conductor_execution_closeout.py",
            "mutations": [],
        }
    try:
        completed = subprocess.run(
        ...
        return {
            "status": "FAIL",
            "reason_code": "CONDUCTOR_EXECUTION_CLOSEOUT_VALIDATOR_ERROR",
...
def evaluate_progress(root: Path, *, run_id: str | None = None) -> dict[str, Any]:
    ...
    if run_root is None:
        return result(
            state="READY_TO_INITIALIZE",
            reason_code="CONDUCTOR_NO_ACTIVE_RUN",
    ...
    if contradiction:
        ...
        return result(
            state="BLOCKED",
            reason_code="CONDUCTOR_EVIDENCE_CONTRADICTION",
    ...
    if recall_is_weak(run_root):
        return result(
            state="BLOCKED",
            reason_code="CONDUCTOR_WEAK_RECALL",
    ...
    if lock_problem:
        return result(
            state="BLOCKED",
            reason_code="CONDUCTOR_INTENT_NOT_LOCKED",
    ...
    if "I2" in stages:
        ...
        if audit_verdict not in {"PASS", "CONDITIONAL PASS"}:
            return result(
                state="BLOCKED",
                reason_code="CONDUCTOR_PACK_AUDIT_NOT_PASSING",
        ...
        if common["execution_mode"] == "EXECUTION_ENABLED" and not has_human_go:
            return result(
                ...
                reason_code="CONDUCTOR_HUMAN_GO_REQUIRED",
        ...
        if closeout is not None:
            if closeout.get("status") != "PASS":
                return result(
                    state="BLOCKED",
                    reason_code="CONDUCTOR_EXECUTION_CLOSEOUT_INVALID",
    ...
    return result(
        ...
        reason_code="CONDUCTOR_NEXT_STAGE_READY",
...
def load_installation_state(root: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        raise ValueError("CONDUCTOR_INSTALLATION_STATE_INVALID") from error
    if isinstance(state, dict) and "conductor_version" not in state and isinstance(state.get("factory_version"), str):
        # Factory-lineage installs (<= 0.2.5) recorded the version under the old key.
        state["conductor_version"] = state["factory_version"]
    if (
        not isinstance(state, dict)
        or state.get("schema_version") != 1
        or not isinstance(state.get("conductor_version"), str)
        ...
        raise ValueError("CONDUCTOR_INSTALLATION_STATE_INVALID")
    return state
...
def load_payload(payload_root: Path) -> tuple[str, list[dict[str, str]]]:
    ...
    if not ownership_path.is_file():
        raise ValueError("CONDUCTOR_PAYLOAD_MANIFEST_MISSING")
    ownership = json.loads(ownership_path.read_text(encoding="utf-8"))
    if ownership.get("schema_version") != 1:
        raise ValueError("CONDUCTOR_PAYLOAD_MANIFEST_INVALID")
    ...
    for entry in entries:
        ...
        if not source.is_file() or file_sha256(source) != expected_digest:
            raise ValueError("CONDUCTOR_PAYLOAD_DIGEST_MISMATCH")
...
def safe_target(root: Path, relative_text: str) -> Path:
    ...
    if relative.is_absolute() or not relative.parts or ".." in relative.parts:
        raise ValueError("CONDUCTOR_UNSAFE_PATH")
...
def git_state_digest(git_path: Path) -> str:
    ...
    if not git_path.is_dir():
        raise ValueError("CONDUCTOR_GIT_STATE_INVALID")
    for path in sorted(
        git_path.rglob("*"),
...
def validate_greenfield_preserved_paths(
    ...
    *,
    ...
    try:
        ...
        raise ValueError("CONDUCTOR_PLAN_STALE") from error
    if current != expected:
...
def evaluate_setup_plan(
    ...
    *,
    ...
    if mode not in {"greenfield", "brownfield"}:
        raise ValueError("CONDUCTOR_SETUP_MODE_INVALID")
    problem = environment_problem(
        harness=harness,
    ...
    if mode == "greenfield" and root.is_dir() and installation_state is None:
        ...
        if unexpected and installation_state is None:
            return result(
                state="BLOCKED",
                reason_code="CONDUCTOR_GREENFIELD_NOT_EMPTY",
                next_legal_action=(
    ...
    try:
        ...
        reason_code = (
            str(error)
            if str(error).startswith("CONDUCTOR_")
            else "CONDUCTOR_PAYLOAD_MANIFEST_INVALID"
        )
        return result(
            state="BLOCKED",
            reason_code=reason_code,
    ...
    if conflicts:
        return result(
            state="BLOCKED",
            reason_code="CONDUCTOR_CONFLICT_USER_OWNED",
    ...
    if plan_details["change_plan"]["approval_state"] == "NOT_REQUIRED":
        return result(
            state="NO_CHANGE",
            reason_code="CONDUCTOR_ALREADY_CURRENT",
            next_legal_action="inspect_factory_progress",
            **plan_details,
        )
    return result(
        state="PLAN_READY",
        reason_code="CONDUCTOR_PLAN_READY",
...
def capture_snapshots(root: Path, paths: list[str]) -> dict[str, dict[str, Any]]:
    ...
    for relative in sorted(set(paths)):
        ...
        if target.exists() and not target.is_file():
            raise ValueError("CONDUCTOR_CONFLICT_USER_OWNED")
        snapshots[relative] = {
...
def validate_setup_result(
    ...
    for entry in managed_files:
        ...
        if not target.is_file() or file_sha256(target) != entry["expected_digest"]:
            raise RuntimeError("CONDUCTOR_SETUP_VALIDATION_FAILED")
...
def restore_bootstrap_paths(
    ...
    if git_created:
        if (
            ...
            return "CONDUCTOR_ROLLBACK_GIT_STATE_CHANGED"
...
def apply_setup_plan(
    ...
    *,
    ...
    if approved_plan_id != plan["plan_id"]:
        return result(
            state="BLOCKED",
            reason_code="CONDUCTOR_PLAN_APPROVAL_REQUIRED",
    ...
    if safe_target(root, transaction_path).exists():
        return result(
            state="BLOCKED",
            reason_code="CONDUCTOR_TRANSACTION_RECEIPT_EXISTS",
    ...
    installation_state = {
        ...
        "source_revision": f"conductor-plugin@{plan['plugin_version']}",
    ...
    try:
        ...
        return result(
            state="BLOCKED",
            reason_code=rollback_reason or "CONDUCTOR_SETUP_ABORTED",
    ...
    return result(
        state="APPLIED",
        reason_code="CONDUCTOR_SETUP_APPLIED",
...
def semver(value: str) -> tuple[int, int, int]:
    ...
    if len(parts) != 3 or any(not part.isdigit() for part in parts):
        raise ValueError("CONDUCTOR_VERSION_INVALID")
    ...
    *,
    ...
    try:
        ...
        if installation_state is None:
            raise ValueError("CONDUCTOR_INSTALLATION_STATE_MISSING")
        ...
        installed_version = installation_state["conductor_version"]
        if semver(target_version) < semver(installed_version):
            return result(
                state="BLOCKED",
                reason_code="CONDUCTOR_DOWNGRADE_UNSUPPORTED",
        ...
        reason_code = (
            str(error)
            if str(error).startswith("CONDUCTOR_")
            else "CONDUCTOR_INSTALLATION_STATE_INVALID"
        )
        return result(
            state="BLOCKED",
            reason_code=reason_code,
    ...
    return result(
        state="PLAN_READY",
        reason_code="CONDUCTOR_UPDATE_REVIEW_REQUIRED",
...
def apply_update_plan(
    ...
    *,
    ...
    if interrupt_after_staging:
        return result(
            state="ROLLED_BACK",
            reason_code="CONDUCTOR_UPDATE_ABORTED",
    ...
    try:
        if safe_target(root, transaction_path).exists():
            raise ValueError("CONDUCTOR_TRANSACTION_RECEIPT_EXISTS")
        ...
        reason_code = (
            str(error)
            if isinstance(error, ValueError) and str(error).startswith("CONDUCTOR_")
            else "CONDUCTOR_UPDATE_ABORTED"
        )
        return result(
            state="ROLLED_BACK",
            reason_code=reason_code,
    ...
    return result(
        state="APPLIED",
        reason_code="CONDUCTOR_UPDATE_APPLIED",
...
def apply_rollback(root: Path, *, approved: bool) -> dict[str, Any]:
    ...
    try:
        installation_state = load_installation_state(root)
        if installation_state is None:
            raise ValueError("CONDUCTOR_ROLLBACK_UNAVAILABLE")
        ...
        if not approved:
            return result(
                state="BLOCKED",
                reason_code="CONDUCTOR_ROLLBACK_APPROVAL_REQUIRED",
        ...
        reason_code = (
            str(error)
            if str(error).startswith("CONDUCTOR_")
            else "CONDUCTOR_ROLLBACK_EVIDENCE_MISMATCH"
        )
        return result(
            state="BLOCKED",
            reason_code=reason_code,
    ...
    return result(
        state="ROLLED_BACK",
        reason_code="CONDUCTOR_ROLLBACK_APPLIED",
        next_legal_action="run_factory_doctor_and_validation",
...
def apply_greenfield_rollback(
    ...
    *,
    ...
    if (
        transaction.get("target_version") != installation_state["conductor_version"]
        or transaction.get("recovery_status") != "AVAILABLE"
    ...
    if bootstrap["git_created"]:
        ...
        if (
            ...
            return result(
                state="BLOCKED",
                reason_code="CONDUCTOR_ROLLBACK_GIT_STATE_CHANGED",
    ...
    try:
        if bootstrap["git_created"]:
            if git_state_digest(git_path) != bootstrap["git_post_init_digest"]:
                raise ValueError("CONDUCTOR_ROLLBACK_GIT_STATE_CHANGED")
            shutil.rmtree(git_path)
        if bootstrap["root_created"]:
            root.rmdir()
    except Exception:
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="conductor-plugin")
    parser.add_argument(
        "--root",
        type=Path,
