...
```bash
python3 scripts/verify_conductor_cli_rollout.py \
...
4. For a new project, open the intended empty directory and invoke
   `$conductor-greenfield`. Review the exact plan and apply only after approving
   its full plan ID; then invoke `$conductor-doctor`, `$conductor-validate`, and
   `$conductor-progress`.
5. For an existing Git repository, invoke `$conductor-doctor`, then
...
For an existing Git repository, invoke `/conductor:doctor` and then
`/conductor:brownfield` instead of Greenfield.
...
2. Run Core knowledge lint. If the project declares
   `docs/Conductor/PROJECT_PREFLIGHT.json`, run the fixed project preflight next.
3. Refresh context recall, then invoke progress.
4. Invoke `$conductor-run` in Codex or `/conductor:run` in Claude.
...
8. After approved execution, retain evidence for every enabled verification
   ...
   `docs/Conductor/templates/EXECUTION_CLOSEOUT_TEMPLATE.json`, and record it with:

```bash
./scripts/conductorctl execution-closeout --run <RUN_ID> --input <DRAFT.json> --json
```
...
Do not retry blindly after a blocked result. Use the reason code and the [troubleshooting guide](CONDUCTOR_PLUGIN_TROUBLESHOOTING.md). For update recovery, use the [rollback guide](CONDUCTOR_PLUGIN_ROLLBACK.md).
