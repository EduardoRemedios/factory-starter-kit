...
conductorctl kilo-stage = bounded stage launcher
...
Recommended:
- Run `conductorctl kilo-stage` from a neutral shell, Codex, or another non-Kilo orchestrator.
- Let `conductorctl kilo-stage` invoke exactly one `kilo run` process for the selected Factory stage.
- Wait for that command to exit before starting the next stage.

Avoid:
- Running `conductorctl kilo-stage` from inside an active Kilo Code session.
...
```bash
./scripts/conductorctl kilo-stage \
  --run <RUN_ID> \
  --stage B \
...
Kilo Native Mode, where an interactive Kilo session owns the whole Factory run and switches active models in-session, is a different harness pattern. It is not implemented by `conductorctl kilo-stage`.
...
```bash
./scripts/conductorctl kilo-stage --run <RUN_ID> --stage B --model kilo/anthropic/claude-opus-4.8 --variant high --auto --timeout-seconds 900
./scripts/conductorctl stage-lint --run <RUN_ID> --stage B

./scripts/conductorctl kilo-stage --run <RUN_ID> --stage C --model kilo/openai/gpt-5.5 --variant high --auto --timeout-seconds 900
./scripts/conductorctl stage-lint --run <RUN_ID> --stage C
```
...
7. writes a JSON run record under `docs/Conductor/runs/<RUN_ID>/kilo_stage_runs/`

The generated worker prompt also tells Kilo not to launch `kilo`, `kilocode`, or `conductorctl kilo-stage` from inside the worker lane.
...
For planning-only Factory stages, Kilo should be allowed to read the repo and write only the run artifacts for the current stage. The `conductorctl kilo-stage` runner checks this after the process exits, but it is not a sandbox.
...
./scripts/conductorctl kilo-stage \
  ...
  --variant high \
  --auto
./scripts/conductorctl stage-lint --run RUN_YYYYMMDD_HHMM_factory --stage B
```
...
./scripts/conductorctl kilo-stage \
  ...
  --variant high \
  --auto
./scripts/conductorctl stage-lint --run RUN_YYYYMMDD_HHMM_factory --stage C
```
...
kilo run \
  ...
  --variant high \
  --format json \
  "Review docs/Conductor/runs/RUN_YYYYMMDD_HHMM_factory/pack/intent.md and intent_redteam.md. Do not edit files. Return only blocking concerns for Purple Gate."
```
...
- Do not start Stage C until the Stage B `conductorctl kilo-stage` process has exited and `stage-lint --stage B` passes.
