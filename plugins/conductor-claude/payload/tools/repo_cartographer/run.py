...
APPROVED_VERIFICATION_COMMANDS: dict[str, tuple[str, ...]] = {
    ...
    "conductorctl_help": ("./scripts/conductorctl", "--help"),
}
...
def _scan_topology(root: Path) -> dict[str, object]:
    ...
    conductor_runs = list((root / "docs/Conductor/runs").glob("RUN_*")) if (root / "docs/Conductor/runs").exists() else []
    return {
        "top_level_dirs": top_level_dirs,
        "python_file_count": len(python_files),
        "test_file_count": len(tests),
        "conductor_run_count": len(conductor_runs),
        "scripts_present": (root / "scripts").exists(),
        "docs_present": (root / "docs").exists(),
        "conductor_present": (root / "docs/Conductor").exists(),
    }
...
def _harvest_verification(root: Path) -> dict[str, object]:
    ...
    return {
        ...
        "conductorctl_present": (root / "scripts/conductorctl").exists(),
...
def _scan_traceability(root: Path) -> dict[str, object]:
    conductor_runs = root / "docs/Conductor/runs"
    traceability_files = list(conductor_runs.rglob("traceability_matrix.md")) if conductor_runs.exists() else []
    verification_plans = list(conductor_runs.rglob("verification_plan.md")) if conductor_runs.exists() else []
    manifests = list(conductor_runs.rglob("verification_manifest.yaml")) if conductor_runs.exists() else []
    return {
    ...
    if int(traceability.get("traceability_matrix_count", 0)) == 0:
        findings.append(
            Finding(
                ...
                severity="INFO",
                confidence="HIGH",
                source_paths=("docs/Conductor/runs/",),
                commit=commit,
...
def _repo_report(metadata: dict[str, object], topology: dict[str, object], short_commit: str) -> str:
    return (
        ...
        f"- Factory runs: {topology['conductor_run_count']}\n"
        f"- Scripts/docs/Conductor present: {topology['scripts_present']}/{topology['docs_present']}/{topology['conductor_present']}\n"
    )
...
def _verification_report(
    ...
    return (
        ...
        f"- Factoryctl present: {verification['conductorctl_present']}\n"
