...
from conductor_host_capability import ARTIFACT_NAME as HOST_CAPABILITIES_ARTIFACT
from conductor_host_capability import validate_artifact as validate_host_capabilities
...
def evaluate_host_capability_contract(root: Path, run_root: Path) -> dict[str, Any]:
    ...
    if requirements["errors"]:
        return {
            "status": "BLOCKED",
            "reason_code": "CONDUCTOR_HOST_CAPABILITY_DECLARATION_INVALID",
    ...
    if not requirements["required"] and not artifact.exists():
        return {
            "status": "PASS",
            "reason_code": "CONDUCTOR_HOST_CAPABILITIES_NOT_REQUIRED",
            "errors": [],
            **requirements,
        }
    ...
    if missing_ids or unmatched_paths:
        ...
        return {
            ...
            **validation,
            "status": "BLOCKED",
            "reason_code": "CONDUCTOR_HOST_CAPABILITY_DECLARATION_MISMATCH",
            "errors": errors,
        }
    return {**requirements, **validation}
...
def check_host_capability_contract(
    *,
    ...
    if result["status"] == "PASS":
        return
    if (
        result["reason_code"] == "CONDUCTOR_HOST_CAPABILITIES_DEFERRED_TARGET"
