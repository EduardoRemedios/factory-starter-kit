...
DEFAULT_DB_ENV_VAR = "CONDUCTOR_CONTEXT_DB_PATH"
LEGACY_DB_PATH = Path("artifacts/conductor_context/context.sqlite3")
...
def default_context_db_path() -> Path:
    ...
    return Path("/tmp/conductor_starter_kit_context/context.sqlite3")
...
def _classify_source(rel_path: str, text: str) -> dict[str, Any]:
    ...
    if len(path_parts) >= 4 and path_parts[:3] == ("docs", "Conductor", "runs"):
        run_id = path_parts[3]
        artifact_type = "conductor_run_pack_artifact" if "pack" in path_parts else "conductor_run_root_artifact"
    elif len(path_parts) >= 4 and path_parts[:3] == ("docs", "Conductor", "missions"):
        mission_id = path_parts[3]
        artifact_type = "mission_artifact"
    elif len(path_parts) >= 5 and path_parts[:4] == ("docs", "Conductor", "ProductOwner", "phases"):
