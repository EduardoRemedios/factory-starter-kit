...
def artifact_path(root: Path, run_id: str) -> Path:
    return root / "docs" / "Conductor" / "runs" / run_id / "pack" / ARTIFACT_NAME
...
def validate_document(
    ...
    *,
    ...
    if not isinstance(document, dict):
        return _result(
            status="BLOCKED",
            reason_code="CONDUCTOR_HOST_CAPABILITY_SCHEMA_INVALID",
            run_id=run_id,
    ...
    if posture == "DEFERRED_TARGET":
        return _result(
            status="BLOCKED",
            reason_code="CONDUCTOR_HOST_CAPABILITIES_DEFERRED_TARGET",
            run_id=run_id,
            posture=posture,
    ...
    if host != current:
        return _result(
            status="BLOCKED",
            reason_code="CONDUCTOR_HOST_CAPABILITY_HOST_MISMATCH",
            run_id=run_id,
            posture=posture,
            capability_count=len(valid_entries),
        )
    ...
    if capability_errors:
        reason_code = "CONDUCTOR_HOST_CAPABILITY_PATH_MISSING"
        if any("KIND_MISMATCH" in item for item in capability_errors):
            reason_code = "CONDUCTOR_HOST_CAPABILITY_KIND_MISMATCH"
        elif any("NOT_EXECUTABLE" in item for item in capability_errors):
            reason_code = "CONDUCTOR_HOST_CAPABILITY_NOT_EXECUTABLE"
        return _result(
            status="BLOCKED",
            reason_code=reason_code,
            run_id=run_id,
    ...
    return _result(
        status="PASS",
        reason_code="CONDUCTOR_HOST_CAPABILITIES_VALID",
        run_id=run_id,
        posture=posture,
        capability_count=len(valid_entries),
    )
...
def validate_artifact(root: Path, run_id: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        return _result(
            status="BLOCKED",
            reason_code="CONDUCTOR_HOST_CAPABILITIES_MISSING",
            run_id=run_id,
...
def discover_artifact(
    ...
    if candidate != expected or candidate.is_symlink() or not candidate.parent.is_dir():
        return _result(
            status="BLOCKED",
            reason_code="CONDUCTOR_HOST_CAPABILITY_OUTPUT_INVALID",
            run_id=run_id,
