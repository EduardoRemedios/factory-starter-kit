...
MISSION_ROOT="docs/Conductor/missions/$MISSION_ID"
...
for line in table_lines[2:]:
    ...
    pack_path_match = re.search(r"docs/Conductor/runs/RUN_\d{8}_\d{4}_[A-Za-z0-9_-]+/pack", cells[header_index["Pack Path"]])
    units.append(
        {
...
for unit in units:
    ...
    expected_pack = f"docs/Conductor/runs/{unit['run_id']}/pack"
    ...
    run_root = root / "docs" / "Conductor" / "runs" / unit["run_id"]
...
if next_planned:
    ...
    if predecessor_index >= 0:
        ...
        if predecessor["status"] == "pack_complete" and predecessor["run_id"]:
            exec_mode = root / "docs" / "Conductor" / "runs" / predecessor["run_id"] / "EXECUTION_MODE.txt"
