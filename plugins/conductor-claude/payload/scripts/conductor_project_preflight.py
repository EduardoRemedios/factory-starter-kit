...
DECLARATION_PATH = Path("docs/Conductor/PROJECT_PREFLIGHT.json")
PROJECT_COMMAND = Path("scripts/conductor_project_preflight")
...
def _load_declaration(root: Path) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    ...
    if not path.exists():
        return None, None
    if not path.is_file():
        return None, _result("CONDUCTOR_PROJECT_PREFLIGHT_DECLARATION_INVALID")
    try:
...
def _validate_project_output(root: Path, stdout: bytes) -> dict[str, Any]:
    try:
        ...
        return _result("CONDUCTOR_PROJECT_PREFLIGHT_OUTPUT_MALFORMED")
    ...
    if not isinstance(payload, dict) or set(payload) != {
        ...
        return _result("CONDUCTOR_PROJECT_PREFLIGHT_OUTPUT_AMBIGUOUS")
    ...
    for value in evidence_paths:
        ...
        if relative.is_absolute() or ".." in relative.parts:
            return _result("CONDUCTOR_PROJECT_PREFLIGHT_EVIDENCE_PATH_INVALID")
    ...
    if payload["status"] == "FAIL":
        return _result(
            "CONDUCTOR_PROJECT_PREFLIGHT_FAILED",
            project_reason_code=project_reason,
            evidence_paths=normalized_paths,
        )
    return _result(
        "CONDUCTOR_PROJECT_PREFLIGHT_PASS",
        outcome="pass",
        project_reason_code=project_reason,
        evidence_paths=normalized_paths,
    )
...
def run_project_preflight(root: Path, run_id: str) -> dict[str, Any]:
    ...
    if declaration is None:
        return _result(
            "CONDUCTOR_PROJECT_PREFLIGHT_NOT_DECLARED",
            outcome="pass",
    ...
    if not command.exists() or not command.is_file():
        return _result("CONDUCTOR_PROJECT_PREFLIGHT_COMMAND_MISSING")
    if command.is_symlink() or not os.access(command, os.X_OK):
        return _result("CONDUCTOR_PROJECT_PREFLIGHT_COMMAND_INVALID")
    ...
    if failure == "timeout":
        return _result("CONDUCTOR_PROJECT_PREFLIGHT_TIMEOUT", **metadata)
    if failure == "output_too_large":
        return _result("CONDUCTOR_PROJECT_PREFLIGHT_OUTPUT_TOO_LARGE", **metadata)
    if return_code != 0:
        return _result(
            "CONDUCTOR_PROJECT_PREFLIGHT_EXIT_NONZERO",
