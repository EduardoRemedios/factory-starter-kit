#!/usr/bin/env bash
# Project merge preflight per docs/Conductor/MERGE_PROTOCOL.md.
...
pack_lint_all_runs() {
  if [ ! -d docs/Conductor/runs ]; then
    echo "no Factory runs present; nothing to pack-lint"
    return 0
  fi
  local failed=0 run_dir
  for run_dir in docs/Conductor/runs/*/; do
    [ -d "$run_dir" ] || continue
    if ./scripts/conductorctl pack-lint --run "$(basename "$run_dir")"; then
...
check full_test_suite ./scripts/conductor-python -m unittest discover -s tests -p 'test_*.py'
