---
name: factory-bmad-bootstrap
description: Preview or apply a pinned BMAD 6.10.0 Core+BMM Claude Code bootstrap with exact approval, audit, and receipts.
---
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/factory_bmad.py" --root . bootstrap --harness claude
```
...
run `/factory-bmad:doctor` in that fresh session; do not imply that the current
