...
def _load_policy():
    ...
    path = Path(__file__).with_name("factory_bmad_policy.py")
    spec = importlib.util.spec_from_file_location("factory_bmad_policy", path)
...
def safe_relative(root: Path, value: str, *, prefix: Path | None = None) -> tuple[Path, Path]:
    ...
    if relative.is_absolute() or ".." in relative.parts:
        raise CompanionError("FACTORY_BMAD_PATH_INVALID", value)
    candidate = root / relative
    cursor = root
    for part in relative.parts:
        cursor = cursor / part
        if cursor.is_symlink():
            raise CompanionError("FACTORY_BMAD_SYMLINK_REJECTED", relative.as_posix())
    resolved = candidate.resolve(strict=False)
    try:
        normalized = resolved.relative_to(root)
    except ValueError as error:
        raise CompanionError("FACTORY_BMAD_PATH_ESCAPE", value) from error
    if prefix is not None and normalized.parts[: len(prefix.parts)] != prefix.parts:
        raise CompanionError("FACTORY_BMAD_PATH_PREFIX_INVALID", value)
...
def bmad_manifest_path(root: Path) -> Path | None:
    ...
    if len(found) > 1:
        raise CompanionError("FACTORY_BMAD_MANIFEST_AMBIGUOUS")
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if name:
            ...
            if current in modules:
                raise CompanionError("FACTORY_BMAD_MODULE_DUPLICATE", current)
            modules[current] = "unknown"
        elif version and current:
            modules[current] = version.group(1)
    if not modules:
        raise CompanionError("FACTORY_BMAD_MODULES_UNREADABLE")
    return modules


def factory_state(root: Path) -> tuple[bool, bool]:
    factory = (root / "docs/Factory/ARCHITECTURE.md").is_file() and (root / "scripts/factoryctl").is_file()
    partial = (root / "docs/Factory").exists() and not factory
...
def doctor(root: Path, harness: str) -> dict[str, Any]:
    ...
    if factory:
        return result("FACTORY_ONLY", "FACTORY_BMAD_FACTORY_ONLY", "preview_pinned_bmad_bootstrap", evidence=evidence)
    if bmad:
        return result("BMAD_ONLY", "FACTORY_BMAD_BMAD_ONLY", "run_factory_brownfield_preview", evidence=evidence)
    entries = meaningful_entries(root)
    if entries:
        return result("NEITHER_BROWNFIELD", "FACTORY_BMAD_NEITHER_BROWNFIELD", "run_factory_brownfield_preview", evidence={**evidence, "meaningful_entries": entries})
    return result("NEITHER_GREENFIELD", "FACTORY_BMAD_NEITHER_GREENFIELD", "run_factory_greenfield_preview", evidence=evidence)
...
def bootstrap_plan(root: Path, harness: str) -> dict[str, Any]:
    ...
    if state["state"] != "FACTORY_ONLY":
        raise CompanionError("FACTORY_BMAD_BOOTSTRAP_STATE_INVALID", state["state"])
    command = [
...
def bootstrap(root: Path, harness: str, approval: str | None) -> dict[str, Any]:
    ...
    if approval is None:
        return result("PLAN_READY", "FACTORY_BMAD_BOOTSTRAP_PLAN_READY", "review_and_exactly_approve_plan", plan=plan)
    ...
    if outcome != "APPLIED":
        return result("BLOCKED", "FACTORY_BMAD_BOOTSTRAP_POST_AUDIT_FAILED", "inspect_receipt_and_recover_manually", receipt=receipt.relative_to(root).as_posix(), unexpected_paths=unexpected, mutations=changes)
...
def promote(root: Path, args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.approve_plan is None:
        return result("PLAN_READY", "FACTORY_BMAD_PROMOTION_PLAN_READY", "review_and_exactly_approve_plan", plan={**plan, "aggregate_sha256": manifest["aggregate_sha256"]})
...
def rollback_plan(root: Path, receipt_value: str) -> tuple[dict[str, Any], Path, Path]:
    ...
    if not receipt_path.is_file():
        raise CompanionError("FACTORY_BMAD_RECEIPT_MISSING")
    try:
        receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    except (OSError, UnicodeError, json.JSONDecodeError) as error:
        raise CompanionError("FACTORY_BMAD_RECEIPT_INVALID") from error
    ...
    if not isinstance(snapshot_value, str) or not isinstance(files, dict):
        raise CompanionError("FACTORY_BMAD_RECEIPT_INVALID")
    ...
    try:
        ...
        actual = {}
    if actual != files:
        raise CompanionError("FACTORY_BMAD_ROLLBACK_DIGEST_MISMATCH")
...
def rollback(root: Path, receipt_value: str, approval: str | None) -> dict[str, Any]:
    ...
    if approval is None:
        return result("PLAN_READY", "FACTORY_BMAD_ROLLBACK_PLAN_READY", "review_and_exactly_approve_plan", plan=plan)
    if approval != plan["plan_id"]:
        return result("BLOCKED", "FACTORY_BMAD_PLAN_APPROVAL_MISMATCH", "review_current_plan", plan=plan)
    ...
    return result("APPLIED", "FACTORY_BMAD_ROLLBACK_APPLIED", "retain_receipts_for_audit", receipt=rollback_receipt.relative_to(root).as_posix(), mutations=[plan["snapshot_path"], rollback_receipt.relative_to(root).as_posix()])
...
def intake_sources() -> dict[Path, Path]:
    return {
        ...
        Path("docs/Factory/PROJECT_PREFLIGHT.json"): ADAPTER_ROOT / "PROJECT_PREFLIGHT.json",
        Path("scripts/factory_project_preflight"): ADAPTER_ROOT / "factory_project_preflight",
        Path("scripts/factory_bmad_policy_lint"): ADAPTER_ROOT / "factory_bmad_policy_lint",
        Path("scripts/factory_bmad_policy.py"): Path(__file__).with_name("factory_bmad_policy.py"),
    }
...
def intake(root: Path, harness: str, approval: str | None) -> dict[str, Any]:
    ...
    if audit["state"] != "READY":
        return result("BLOCKED", "FACTORY_BMAD_INTAKE_AUDIT_REQUIRED", "resolve_factory_bmad_audit", audit=audit)
    files = []
    conflicts = []
    ...
    for destination, source in intake_sources().items():
        if not source.is_file():
            return result("BLOCKED", "FACTORY_BMAD_PACKAGE_INCOMPLETE", "repair_companion_package")
        ...
        executable = destination in {
            Path("scripts/factory_project_preflight"),
            Path("scripts/factory_bmad_policy_lint"),
        }
    ...
    if conflicts:
        return result("BLOCKED", "FACTORY_BMAD_INTAKE_CONFLICT", "reconcile_user_owned_files", conflicts=conflicts)
    ...
    if not creates:
        return result("READY", "FACTORY_BMAD_INTAKE_CURRENT", "draft_raw_brief_from_template")
    if approval is None:
        return result("PLAN_READY", "FACTORY_BMAD_INTAKE_PLAN_READY", "review_and_exactly_approve_plan", plan=plan)
    if approval != plan["plan_id"]:
        return result("BLOCKED", "FACTORY_BMAD_PLAN_APPROVAL_MISMATCH", "review_current_plan", plan=plan)
    mutations = []
    for item in creates:
    ...
    return result("APPLIED", "FACTORY_BMAD_INTAKE_APPLIED", "draft_raw_brief_from_template", receipt=receipt.relative_to(root).as_posix(), mutations=mutations)
...
def concise(payload: dict[str, Any]) -> str:
    ...
    if isinstance(evidence, dict):
        lines.append(f"Factory: {'present' if evidence.get('factory_present') else 'absent'}; BMAD: {'present' if evidence.get('bmad_present') else 'absent'}")
...
def main() -> int:
    ...
    try:
        if args.command == "doctor":
            ...
            try:
                ...
                if enforcement_activation(root)["active"]:
                    print("FACTORY_BMAD_HOOK_INPUT_INVALID: malformed hook JSON", file=sys.stderr)
                    return 2
                hook_input = {}
            if not isinstance(hook_input, dict):
                if enforcement_activation(root)["active"]:
                    print("FACTORY_BMAD_HOOK_INPUT_INVALID: hook input must be an object", file=sys.stderr)
                    return 2
                hook_input = {}
        ...
        payload = result("BLOCKED", "FACTORY_BMAD_IO_FAILURE", "inspect_error_and_retry_safely", detail=str(error))
