# docs/Conductor/Spec/STAGE_CONTRACTS.md — Factory Stage Contracts (v4.20)
...
- Pre-run knowledge lint MUST pass (`bash scripts/knowledge_lint.sh`) before `STAGE_A` starts, and output MUST be persisted at `docs/Conductor/runs/<RUN_ID>/KNOWLEDGE_LINT.txt`.
- If `docs/Conductor/PROJECT_PREFLIGHT.json` exists, `./scripts/conductorctl project-preflight --run <RUN_ID>` MUST run after knowledge lint and before context recall; `PROJECT_PREFLIGHT.txt` MUST record `CONDUCTOR_PROJECT_PREFLIGHT_PASS` or initialization halts.
- If the run is advancing a unit inside an already-authorized mission, pre-run mission lint MUST pass (`bash scripts/mission_lint.sh <MISSION_ID>`) before `STAGE_A` starts, and output MUST be persisted at `docs/Conductor/runs/<RUN_ID>/MISSION_LINT.txt`.
...
- After each stage writes its handoff, run `./scripts/conductorctl stage-lint --run <RUN_ID> --stage <STAGE_CODE>`. If stage-lint fails, do not advance to the next stage until the handoff or expected stage output is fixed.
...
- `STAGE_A` may not start until `docs/Conductor/runs/<RUN_ID>/CONTEXT_RECALL_REPORT.md` exists and was generated from the artifact recall index via `./scripts/conductorctl context-report --profile stage-a`.
...
- Mission checkpointing and downstream mission-unit planning may not proceed until `docs/Conductor/missions/<MISSION_ID>/MISSION_CONTEXT_RECALL_REPORT.md` exists and was generated via `./scripts/conductorctl context-report --profile mission-checkpoint`.
- PO sprint brief drafting and review must follow the PO process recall contract in `docs/Conductor/ProductOwner/PO_PROCESS.md`; Factory stage execution assumes that upstream brief-cycle recall evidence already exists when the raw brief originated from the PO lane.
- Context recall artifacts are evidence aids, not authority. Source artifacts remain canonical.

## Cross-run memory rule (HARD)
- Mandatory pre-run cross-run memory is limited to `docs/Conductor/SCRATCHPAD.md` `## Active Pitfalls (Mandatory)`.
- Per-run retrospectives live in `docs/Conductor/runs/<RUN_ID>/RETRO.md` and are optional context unless explicitly required by a stage brief.

## Run structure (HARD)
A run produces:
- `docs/Conductor/runs/<RUN_ID>/raw_brief.md`
- `docs/Conductor/runs/<RUN_ID>/RETRO.md`
- `docs/Conductor/runs/<RUN_ID>/KNOWLEDGE_LINT.txt`
- `docs/Conductor/runs/<RUN_ID>/CONTEXT_RECALL_REPORT.md`
- `docs/Conductor/runs/<RUN_ID>/EXECUTION_MODE.txt`
- `docs/Conductor/runs/<RUN_ID>/SPRINT_ID.txt`
- `docs/Conductor/runs/<RUN_ID>/PROJECT_PREFLIGHT.txt` (required only when the project declares `docs/Conductor/PROJECT_PREFLIGHT.json`)
- `docs/Conductor/runs/<RUN_ID>/pack/`
- `docs/Conductor/runs/<RUN_ID>/EXECUTION_AUTHORIZATION.md` (required only for a post-I2 cross-mode activation)
- `docs/Conductor/runs/<RUN_ID>/EXECUTION_PROMPT.md` (required only when `EXECUTION_MODE.txt = EXECUTION_ENABLED` after `STAGE_I2` PASS plus human GO)

If Mission Mode is enabled, mission root additionally produces:
- `docs/Conductor/missions/<MISSION_ID>/MISSION_MANIFEST.md`
- `docs/Conductor/missions/<MISSION_ID>/MISSION_CONTEXT_RECALL_REPORT.md`
- `docs/Conductor/missions/<MISSION_ID>/MISSION_CHECKPOINT.md`
- `docs/Conductor/missions/<MISSION_ID>/MISSION_COMPLETION_REPORT.md`
- `docs/Conductor/missions/<MISSION_ID>/MISSION_EXECUTION_PROMPT.md` (optional)
- `docs/Conductor/missions/<MISSION_ID>/MISSION_CURSOR.json` (optional derived resume cursor for Codex Mission Goal Continuity adapter)

If the run is advancing a unit inside an already-authorized mission, run root additionally produces:
- `docs/Conductor/runs/<RUN_ID>/MISSION_LINT.txt`
...
Command:
- `./scripts/conductorctl stage-lint --run <RUN_ID> --stage <STAGE_CODE>`
...
- `DISK`: `PROJECT_PREFLIGHT.txt` (only when `docs/Conductor/PROJECT_PREFLIGHT.json` exists)
...
- if `docs/Conductor/PROJECT_PREFLIGHT.json` exists, `PROJECT_PREFLIGHT.txt` exists and records `project_preflight: PASS` plus `CONDUCTOR_PROJECT_PREFLIGHT_PASS`
...
- if `verification_manifest.yaml` exists, it is valid per `VERIFICATION_MANIFEST_TEMPLATE.yaml` and `conductorctl pack-lint`
...
- `DISK`: `pack/intent.md`, `pack/risk_register.md`, `pack/PACK_AUDIT_REPORT.md`, `docs/Conductor/SCRATCHPAD.md`

Outputs:
- `EXECUTION_PROMPT.md`

Exit criteria:
- prompt is instantiated from `docs/Conductor/templates/EXECUTION_PROMPT_TEMPLATE.md`
...
Contract:
- canonical path: `docs/Conductor/runs/<RUN_ID>/EXECUTION_CLOSEOUT.json`
- schema: `conductor.execution-closeout.v1`
- author the outcome outside the validator, then record it only with
  `./scripts/conductorctl execution-closeout --run <RUN_ID> --input <DRAFT> --json`
...
- `DISK`: `docs/Conductor/MISSION_MODE.md`
