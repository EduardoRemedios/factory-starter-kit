...
```text
Read AGENTS.md, docs/Conductor/ORCHESTRATION.md, and docs/Conductor/SCRATCHPAD.md Active Pitfalls.
...
2. `./scripts/conductorctl context-index`
3. `./scripts/conductorctl context-report --profile stage-a --scope <RUN_ID> --output docs/Conductor/runs/<RUN_ID>/CONTEXT_RECALL_REPORT.md`
4. run stages `A -> I2`, with `./scripts/conductorctl stage-lint --run <RUN_ID> --stage <STAGE>` after each handoff
5. `./scripts/conductorctl pack-lint --run <RUN_ID>`
...
- `conductor-root-planner`
- `factory-intent-contractor`
- `conductor-purple-gate`
- `conductor-pack-consolidator`
- `conductor-execution-closeout`
...
type = "command"
command = '/usr/bin/python3 "$(git rev-parse --show-toplevel)/.codex/hooks/conductor_post_tool_use.py"'
timeout = 30
