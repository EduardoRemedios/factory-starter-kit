...
OUTCOME_RULES = {
    "REVIEW_READY": (
        "CONDUCTOR_EXECUTION_REVIEW_READY",
        "review_the_retained_clean_worktree",
    ),
    "NO_GO": (
        "CONDUCTOR_EXECUTION_NO_GO",
        "review_failed_verification_evidence",
    ),
    "BLOCKED": (
        "CONDUCTOR_EXECUTION_BLOCKED",
...
def exact_object(value: Any, keys: set[str], label: str) -> dict[str, Any]:
    if not isinstance(value, dict) or set(value) != keys:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", f"{label}_keys")
    return value
...
def unique_strings(value: Any, label: str) -> list[str]:
    if (
        ...
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_DUPLICATE_ID", label)
    return value


def safe_run_root(root: Path, run_id: str) -> Path:
    if not RUN_ID_RE.fullmatch(run_id):
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_IDENTITY_MISMATCH", "run_id")
    cursor = root.resolve()
    for part in ("docs", "Conductor", "runs", run_id):
        cursor = cursor / part
        if cursor.is_symlink():
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_UNSAFE_PATH", "run_root")
    if not cursor.is_dir():
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_IDENTITY_MISMATCH", "run_root")
    return cursor


def safe_file(root: Path, relative: Any, label: str) -> Path:
    if not isinstance(relative, str) or not relative or "\\" in relative:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_UNSAFE_PATH", label)
    ...
    if not resolved.is_file() or resolved.is_symlink():
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_EVIDENCE_MISSING", f"{label}:{relative}")
    return resolved


def validate_digest(value: Any, label: str) -> str:
    if not isinstance(value, str) or not SHA256_RE.fullmatch(value):
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", f"{label}_sha256")
    return value
...
def validate_reference(
    ...
    *,
    ...
    if expected_path is not None and reference["path"] != expected_path:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_PIN_MISMATCH", label)
...
def parse_verification_ids(path: Path) -> list[str]:
    ...
    if not ids or len(ids) != len(set(ids)) or not all(ID_RE.fullmatch(item) for item in ids):
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_VERIFICATION_MANIFEST_INVALID", "check_ids")
    return ids
...
def parse_micro_sprint_ids(path: Path) -> list[str]:
    ...
    if not ids or len(ids) != len(set(ids)):
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_MICRO_SPRINT_MISMATCH", "planned_ids")
    return ids
...
def load_closeout(path: Path) -> dict[str, Any]:
    def reject_duplicate_keys(pairs: list[tuple[str, Any]]) -> dict[str, Any]:
        ...
        for key, item in pairs:
            if key in value:
                fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", f"duplicate_key:{key}")
            value[key] = item
        return value

    try:
        if path.stat().st_size > 1024 * 1024:
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", "oversize")
        value = json.loads(
            path.read_text(encoding="utf-8"),
            object_pairs_hook=reject_duplicate_keys,
        )
    except (OSError, json.JSONDecodeError) as error:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", "json")
...
def validate_authorization_reference(
    ...
    if reference["path"] != "EXECUTION_AUTHORIZATION.md":
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_PIN_MISMATCH", "execution_authorization")
    ...
    for candidate in sorted(run_root.iterdir()):
        if (
            ...
            return
    fail("CONDUCTOR_EXECUTION_CLOSEOUT_PIN_MISMATCH", "execution_authorization_archived")
...
def validate_closeout(
    ...
    if value["schema"] != SCHEMA:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", "schema")
    ...
    if not sprint_path.is_file() or value["sprint_id"] != sprint_path.read_text(encoding="utf-8").strip():
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_IDENTITY_MISMATCH", "sprint_id")
    ...
    if (
        ...
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_IDENTITY_MISMATCH", "execution_mode")
    outcome = value["outcome"]
    if outcome not in OUTCOME_RULES:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_OUTCOME_CONTRADICTION", "outcome")
    reason_code, next_action = OUTCOME_RULES[outcome]
    if value["reason_code"] != reason_code or value["next_legal_action"] != next_action:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_OUTCOME_CONTRADICTION", "outcome_mapping")
    if value["authority_grants"] != []:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_AUTHORITY_VIOLATION", "authority_grants")
    ...
    if completed != planned_micro_sprints:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_MICRO_SPRINT_MISMATCH", "completed_micro_sprints")

    expected_checks = parse_verification_ids(manifest_path)
    results = value["verification_results"]
    if not isinstance(results, list):
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", "verification_results")
    result_ids: list[str] = []
    non_passing = 0
    for index, item in enumerate(results):
        ...
        if not isinstance(check_id, str) or not ID_RE.fullmatch(check_id):
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", "verification_id")
        result_ids.append(check_id)
        status = result_item["status"]
        if status not in RESULT_STATUSES:
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", "verification_status")
        ...
        if sha256_file(evidence_path) != expected_digest:
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_EVIDENCE_DIGEST_MISMATCH", check_id)
        blocker = result_item["blocker"]
        if status == "PASS" and blocker is not None:
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_OUTCOME_CONTRADICTION", check_id)
        if status != "PASS":
            non_passing += 1
            if not isinstance(blocker, str) or not blocker.strip():
                fail("CONDUCTOR_EXECUTION_CLOSEOUT_OUTCOME_CONTRADICTION", check_id)
    if len(result_ids) != len(set(result_ids)):
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_DUPLICATE_ID", "verification_results")
    if result_ids != expected_checks:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_VERIFICATION_COVERAGE_MISMATCH", "verification_results")
    if outcome == "REVIEW_READY" and non_passing:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_OUTCOME_CONTRADICTION", "review_ready_requires_all_pass")
    if outcome != "REVIEW_READY" and not non_passing:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_OUTCOME_CONTRADICTION", "negative_requires_blocker")
    if outcome == "REVIEW_READY" and any(item["status"] == "NOT_RUN" for item in results):
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_OUTCOME_CONTRADICTION", "not_run")

    retained = value["retained_evidence"]
    if not isinstance(retained, list) or not retained:
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", "retained_evidence")
    ...
    for index, item in enumerate(retained):
        ...
        if not isinstance(evidence["id"], str) or not ID_RE.fullmatch(evidence["id"]):
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_SCHEMA_INVALID", "retained_evidence_id")
        ...
        if sha256_file(evidence_path) != expected_digest:
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_EVIDENCE_DIGEST_MISMATCH", evidence["id"])
    if len(retained_ids) != len(set(retained_ids)) or len(retained_paths) != len(set(retained_paths)):
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_DUPLICATE_ID", "retained_evidence")
...
def validate_closeout_file(root: Path, run_id: str) -> dict[str, Any]:
    ...
    if not path.is_file() or path.is_symlink():
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_MISSING", CLOSEOUT_NAME)
...
def record_closeout(root: Path, run_id: str, input_path: Path) -> dict[str, Any]:
    ...
    if not resolved_input.is_file() or resolved_input.is_symlink():
        fail("CONDUCTOR_EXECUTION_CLOSEOUT_INPUT_MISSING", str(input_path))
    ...
    if target.exists():
        if not target.is_file() or target.is_symlink() or target.read_bytes() != canonical:
            fail("CONDUCTOR_EXECUTION_CLOSEOUT_EXISTS", CLOSEOUT_NAME)
        mutations: list[str] = []
    else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="conductor-execution-closeout")
