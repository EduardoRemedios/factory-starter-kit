...
LOCK_FILENAME = ".conductor_kilo_stage.lock"
EXCLUDED_DIRS = {".git", ".conductor_context", "__pycache__", ".pytest_cache", ".mypy_cache", ".ruff_cache"}
    ...
    *,
    ...
    if not allow_nested_kilo and _running_under_kilo():
        raise FactoryKiloStageError(
            "refusing to run nested Kilo: invoke conductorctl kilo-stage from a neutral shell, Codex, or another non-Kilo orchestrator "
...
- Do not launch `kilo`, `kilocode`, `conductorctl kilo-stage`, or any nested agent process.
...
- docs/Conductor/SCRATCHPAD.md Active Pitfalls only
...
- Do not run Kilo, kilocode, or conductorctl kilo-stage from inside this worker lane.
...
def _resolve_run(root: Path, run: str) -> Path:
    ...
    if not run_candidate.is_absolute():
        ...
        by_id = (root / "docs" / "Conductor" / "runs" / run).resolve()
