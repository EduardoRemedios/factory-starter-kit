...
`python3 "${CLAUDE_PLUGIN_ROOT}/scripts/conductor_plugin.py" --root "$PWD" greenfield --harness claude`.
In Codex, resolve the package containing this `SKILL.md` and run its
`scripts/conductor_plugin.py` by absolute path with
