...
def marketplace_checks(root: Path) -> list[dict[str, str]]:
    ...
    source = load_json(root / "plugin-src/factory/manifest.json")
    package = load_json(root / "plugins/factory-claude/.claude-plugin/plugin.json")
    runtime_version = source_constant(root / "plugin-src/factory/runtime/factory_plugin.py", "PLUGIN_VERSION")
    ...
    if marketplace is None:
        ...
        checks.append(
            check(
                ...
                "PASS" if "factory" in names else "BLOCKED",
