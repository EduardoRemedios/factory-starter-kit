...
- Authored source: `plugin-src/factory/`
- Codex package: `plugins/factory/`
- Claude package: `plugins/factory-claude/`
...
- Claude's desired command `/factory:doctor` uses skill name `doctor`.
- Codex's desired skill `$factory-doctor` uses skill name and directory `factory-doctor`.
...
- Platform package files are generated; edits must occur in `plugin-src/factory/`.

## Validation Commands
- Codex schema: `python3 <plugin-creator-skill-root>/scripts/validate_plugin.py plugins/factory`
- Claude schema: `claude plugin validate plugins/factory-claude --strict`
- Build regression: `python3 -m unittest tests.test_factory_plugin_build -v`
- Live Codex: install from `.agents/plugins/marketplace.json` in the Codex app and start a new task.
- Live Claude: update Claude Code, run `claude --plugin-dir ./plugins/factory-claude`, then invoke `/factory:doctor`.
