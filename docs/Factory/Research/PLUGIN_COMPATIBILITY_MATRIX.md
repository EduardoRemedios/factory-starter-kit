...
| Public invocation | `$factory-doctor` or implicit matching | `/factory:doctor` | One unchanged skill tree cannot provide both preferred names. | High |
...
- Windows and Linux: unverified for the initial pilot; doctor must return `FACTORY_ENVIRONMENT_UNVERIFIED` until VM-009 evidence exists for those environments.
