...
- Factory/Factory-BMAD 0.2.3 is `AMENDED_SOURCE_LIVE_QUALIFIED`: all four
  ...
  decision.
- Closure evidence:
  `artifacts/verification/factory_bmad_023_recovery/VM-008.json`,
  `artifacts/verification/factory_bmad_023_recovery/VM-012.json`,
  `artifacts/verification/factory_bmad_023_recovery/VM-013.json`, and
  `docs/Factory/runs/RUN_20260815_0714_factory_bmad_023_live_recovery/RUN_INTEGRITY_REPAIR.md`.
...
- Observed: Odyssey v2 project preflight returned
  `FACTORY_BMAD_DRAFT_CITATION` because the checklist sentence asserting that no
  draft was cited contained the literal forbidden draft-directory token.
- Source confirmation:
  `plugin-src/factory-bmad/project-adapter/RAW_BRIEF_TEMPLATE.md` contains the
  token and `factory_project_preflight` rejects any occurrence of it.
...
- Priority: P1
- Status: CLOSED
- Observed: the first `/factory-bmad:promote` Skill invocation halted with
  `FACTORY_BMAD_HOOK_INPUT_INVALID`; a later direct evaluator journey succeeded.
...
- Observed: Odyssey v2 Stage D correctly recorded `FAIL` and created no locked
  intent or digest, but `/factory:progress` reported `next_legal_action:
...
- Cause: `factory_stage_lint.py` enforces the 500-word handoff cap but does not
  enforce the per-artifact caps that `factory_pack_lint.py` checks later.
...
- Likely boundary: the installed 0.2.1 project includes `scripts/factory-python`
  with `PYTHONDONTWRITEBYTECODE=1`, but `scripts/factoryctl` itself uses a raw
...
- Source confirmation: working-tree and `HEAD` copies of
  `scripts/factory_pack_lint.py` share SHA-256
...
- Acceptance: a first-time operator can distinguish autocomplete, hook denial,
  and the optional `/factory-bmad:doctor` recovery action.
