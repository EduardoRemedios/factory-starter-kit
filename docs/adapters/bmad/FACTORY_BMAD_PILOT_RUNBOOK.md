...
- Unknown future `bmad-*` names fail closed with
  `FACTORY_BMAD_WORKFLOW_PROHIBITED`.
...
For first-team rollout, use `FACTORY_BMAD_CLI_ROLLOUT_PLAYBOOK.md` as the
operator-facing path and `FACTORY_BMAD_BOOTSTRAP_RECOVERY.md` for any blocked
bootstrap receipt.
