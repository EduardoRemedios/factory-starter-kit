...
|---|---|---|---|
| 1 | **Golden packs.** Copy `RUN_20260902_0725_*` and `RUN_20260903_1750_*` packs to `tests/golden_packs/`; test asserts pack-lint PASS on both. | `tests/` | Test passes; no other change |
| 2 | **Schemas + templates.** Land `docs/Conductor/contracts/*.schema.json` (from this pack), templates, `test_schemas.py` with positive and negative fixtures. | `docs/Conductor/`, `tests/` | Schemas validate templates; nothing consumes them yet |
...
| 11 | **Rename user-facing surfaces.** Plugin display names, slash namespace `/conductor:`, docs titles, README, onboarding. Internal ids unchanged (Open Question 1). | docs, `plugin-src/*/manifest.json`, marketplaces | Docs tests updated; marketplace versions 0.3.0 |
...
| Q4 Disposable repo opened in Cursor | Cursor | AGENTS.md managed block applied; CLI-only operation viable; Team Rules do not override | screenshot + CLI receipts |
