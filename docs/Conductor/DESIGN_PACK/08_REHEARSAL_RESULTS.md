...
| F-7 (**fixed 0.3.2**) | Update from a Factory 0.2.5 install is BLOCKED: legacy state path and managed paths are not discovered | Rename-aware update: discover `docs/Factory/installation/INSTALLATION_STATE.json`, plan the `docs/Factory` → `docs/Conductor` managed-set migration |
