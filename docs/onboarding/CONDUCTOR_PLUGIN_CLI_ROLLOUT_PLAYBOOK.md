...
[`CONDUCTOR_FIRST_TESTER_HANDOFF.md`](CONDUCTOR_FIRST_TESTER_HANDOFF.md) before this
team playbook.
...
3. Install Factory from the supplied marketplace root:
   ...
   claude plugin prune
   /absolute/path/to/factory-starter-kit/scripts/conductor-python \
     /absolute/path/to/factory-starter-kit/scripts/verify_conductor_cli_rollout.py \
     --marketplace-root /absolute/path/to/factory-starter-kit \
     --target-root /absolute/path/to/team/repo \
     --json
...
5. For a new project, invoke `/conductor:greenfield` first.
6. For an existing Git repository, invoke `/conductor:doctor`, then
   `/conductor:brownfield` if Doctor reports Factory is not configured.
7. Approve setup only by quoting the exact full current plan ID.
8. After setup, run `/conductor:doctor`, `/conductor:validate`, and
   `/conductor:progress`.
